...
dateNote: 'Second edition April 19th, 2026'
...
Arguably, many failures of post-Marx socialist experiments came from trying to solve problems created by overly direct value capture with more of the same: replacing markets with bureaucracy and coercion. But neither end of the spectrum inherently requires a totalitarian apparatus to enforce it. Capitalism, too, has no fixed allegiance to closed proprietary systems—the rise of open-source software and the success of cryptocurrencies both point the other way. As value increasingly lives in shared pools of code and models, exclusive IP and closed weights can start to look like friction, while openness may strengthen the network.
