...
Below is an interactive demo of img2bez running entirely in your browser, compiled from Rust to WebAssembly, with a simple React frontend in an [Astro Island](https://docs.astro.build/en/concepts/islands/). Click **Trace** to vectorize a raster image and see the outline img2bez produces; scroll to zoom and drag to pan. To try your own glyph, drop an image onto the app (or use the picker in the corner). This runs with the default settings, which can be tuned for better output depending on image quality and design style.
...
// result.paths is a Vec<kurbo::BezPath>; print each contour's SVG path data
...
Those three stages have a lot of knobs: how sharp a turn has to be to count as a corner, how tight a fit has to be, the margins that gate the refinement pass. I tuned almost none of them by hand. They came out of an eval harness that scores every candidate change against a reference font and is built to be driven by coding agents like Codex, Claude Code, and Hermes. The harness is the next section.
