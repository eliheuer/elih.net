...
<li><a href="#08-an-api-for-tools-and-agents"><span class="n">08</span>An API for Tools and Agents</a></li>
...
The profile matches image quality. **`wild`** (the default) is for unknown rasters like an AI image API's output, the case the demo runs by default; it fits loosely so it does not chase anti-aliasing noise. **`clean`** is for sharp renders of existing fonts and fits tightly. **`photo`** is for soft scans of printed type; it blurs the image first so the trace follows a clean edge instead of every wobble of the ink. The profile is auto-detected and can be forced with `--profile`.
...
The most powerful of these modes is the grid. `--grid` snaps every on-curve point to a coordinate divisible by its spacing, and it defaults to `2`: an even-integer, power-of-two grid. This is the constraint I built [Virtua Grotesk](/blog/virtua-grotesk) on. Every point in that font lands on a power-of-two grid, which keeps the outlines quantized: every coordinate is a round number drawn from a small, fixed set of positions rather than an arbitrary float, so the letters are built from a compact vocabulary of values instead of a smear of decimals. That is what makes the whole family legible as training data for a neural network. The [Virtua Grotesk post](/blog/virtua-grotesk) is the long argument for why that grid matters and what it has to do with the way LLMs compress font sources.
