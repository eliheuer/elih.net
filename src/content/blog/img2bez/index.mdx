...
Below is an interactive demo of img2bez compiled from Rust to WASM. Click "Trace" to vectorize a raster image and see the outline it produces, and drop an image onto the application to try your own glyph. The settings change the output when you click "Trace", but the ones to watch are the **Rules vs Learned** toggles, which swap a structural decision from the procedural rule to a trained ML model. Flipping one lets you watch a small learned model change a structural call live, running client-side and deterministically with no server in the loop.
...
<li><a href="#15-what-the-first-training-runs-found"><span class="n">15</span>What the First Training Runs Found</a></li>
<li><a href="#16-early-days-and-an-invitation"><span class="n">16</span>Early Days, and an Invitation</a></li>
</ol>
</nav>
...
img2bez is designed to be fast and cheap enough to embed directly in a font editor or a SaaS product. You could instead hand the whole job to a large AI model, and pure generative approaches are getting better, but they are slow and expensive to run. A procedural tracer is the opposite: fast, cheap, and deterministic. So img2bez does the bulk of the work up front: the tracer gets roughly 90% of the way, a lightweight AI harness the harder 9%, and a human designer the final 1%. The AI stays small and local: small discriminative models for the cleanup, the first of which now ships (more below), with img2bez-specific LoRAs for open-weight image models planned next.
...
<video src="/demos/img2bez/g-compare.mp4" autoplay loop muted playsinline aria-label="A capital G, at Regular and Bold weights, traced two ways and interpolated between them, both shown in the same point language. Left: vtracer, a general-purpose tracer at its cleanest settings, produces a smooth outline but scatters a couple hundred points along the pixels (210 for this frame), none on the letter's structure, so its corners soften. Right: img2bez places 34 points on the extrema and corners, keeping the crossbar and terminal crisp and the two masters interpolation-compatible." style="display:block;margin-inline:auto;width:100%;"></video>

The same G traced two ways and interpolated between Regular and Bold, both drawn in the same point language: smooth on-curve points are green circles, corners orange squares, off-curve handles purple. On the left, vtracer at its cleanest settings still needs a couple hundred points scattered around the outline, none of them on the letter's structure, so its corners soften and the two weights do not even trace to the same number of points. On the right, img2bez traces the masters jointly: the structure is decided once across the set, so both masters share one point structure with every point on an extremum or a corner, and the crossbar and terminal stay crisp through every intermediate weight.
...
**Optimization.** The first family treats vectorization as an optimization problem, enabled by the *differentiable rasterizer*: a renderer you can backpropagate through, so it reports how to move each control point of an outline to render closer to a target image. That turns the control points into parameters gradient descent can optimize, or that a neural network can learn to produce. [diffvg](https://github.com/BachiLi/diffvg) (2020) is the standard implementation, shipped as a PyTorch operator; img2bez adopts its objective of scoring a candidate outline against the pixels (more below). [LIVE](https://github.com/Picsart-AI-Research/LIVE-Layerwise-Image-Vectorization) builds on diffvg to add paths layer by layer, and newer rasterizers such as [Bézier Splatting](https://arxiv.org/abs/2503.16424) run more than an order of magnitude faster.
...
- a **WASM build** with JavaScript bindings (`traceToJson`, `traceToSvg`, `traceToGlif`, `tracePlaceToJson` for the trace-and-place path, and `traceWithJudge` for the reference-free quality score the demo above prints) for the browser,
...
That single font is where the tracer is tuned tightest, but it is no longer the only thing it is measured against. A separate font-corpus eval renders real, open-licensed fonts to bitmaps, traces them, and scores each trace against the font's *own* outline. The corpus spans 27 masters across more than a dozen families and three scripts (Latin, Arabic, and Hebrew: Rubik, Inter, Amiri, Spectral, Source Serif and Sans, EB Garamond, and more), under a hard rule that no change may improve one family by trading away another. A real font's outline already records where the designer put every point, so this yields ground-truth structure for free, at a scale no hand-drawn reference can reach; per-font mean shape overlap (IoU) currently runs in the high 0.90s. It is also the corpus that trained the first learned head, below.
...
The two have to be balanced, because the interesting failure mode scores well on one and badly on the other. A trace that chases every wobble in a soft scan reproduces the pixels almost perfectly, but with three times the points a designer would use. On a soft photographic `a`, the over-segmented 69-point trace scores *higher* on reproduction than the clean 23-point one, and lower overall, because the structure terms catch what the pixel score rewards. That combined score is the judge, and it now ships in the binary (a port of the eval harness's `score_wild.py`) with a WASM binding the demo at the top of this page calls on every trace: the `judge` value printed under each result is exactly this score.
...
When people hear "machine learning" next to "AI image to font," they reasonably picture something large. The models img2bez trains are the opposite. They are small enough to count the parameters by hand, and they ship as constants inside the binary rather than as files loaded at runtime: hand-rolled tree-walks and matrix multiplies over features the pipeline has already computed, with no ONNX runtime and no `candle` or `tch` dependency, a few nanoseconds per call and bit-for-bit deterministic.

The design is a set of tiny task-specific heads, one at each pipeline gate that used to be a bare threshold comparison: keep or drop a corner candidate, resolve a cusp as a weld or a flat or a smooth point, anchor a point at an inflection or not, read a straight run as a designed line or a curve fragment. Each is a gradient-boosted tree ensemble or a two-layer network of at most a few thousand parameters; the whole set is budgeted to fit in well under 100 KB. Each ships opt-in behind a flag while it is measured against the procedural rule it might replace, and with every head off the output is byte-identical to the hand-tuned pipeline.

The first of these now ships. The corner keep/drop head is trained offline against real font outlines and exported as a `const` array of a couple hundred shallow trees; it rides in the binary behind `IMG2BEZ_CORNER_HEAD`, takes seven precomputed features in and returns one score, and walks its trees in a few nanoseconds. A golden test checks that the Rust tree-walk reproduces the Python model it was trained from to within float noise. It is small enough that "the model" is a header file, and it is the whole thesis made concrete: a learned decision shipped as data inside a deterministic tracer, with no runtime, no GPU, and no change to how the tool is deployed. You can compare the two yourself: the demo at the top of this page has a Corners control that switches between the hand-tuned rule and the learned head on every trace, and the structure view shows where they disagree about points.

The selector is the same shape, still procedural for now. It reads a cheap summary of the input image (edge sharpness, noise, how anti-aliased the edges are, the glyph's size in pixels) and chooses the trace settings. Mapping four numbers to four is a tiny function: as a rule it is a handful of thresholds; learned, it is a two-layer network of about seventy-five weights, a few hundred bytes. The judge from the previous section is the same scale again, a handful of weights balancing reproduction against structure. The corner head is simply the first of these gates where a learned model took over from the rule.

Only one role would ever be large: a model that draws the outline itself, and it is exactly the one img2bez keeps out of the loop. A network that emits curves is stochastic and heavy, the opposite of what a type designer needs from a tracing tool, where the same input has to give the same output every time and run locally. So the drawing stays procedural and the learned parts only decide.

The deciders can be small because the hard work is already done. The tracer turns pixels into clean curves on its own; what is left for a model is a low-dimensional nudge (keep this point or not, fit tighter or looser) from a cheap description of the image. The contrast is worth stating plainly: the image you drop in may have been generated by a model with billions of parameters, and the models that decide how to trace it have a few thousand. One is a probabilistic artist, the others deterministic dispatchers, and keeping them on opposite sides of that line is the point.

There is a standard name for this distinction. A model that produces new content is generative; a model that takes content and returns a label, a score, or a choice is discriminative. The learned parts of img2bez are all discriminative. The selector reads image features and returns settings; the judge of the previous section reads a trace and returns a quality number. The judge is an instance of what the AI-engineering literature calls an AI judge or evaluator model, the familiar idea of using one model to grade another's output, scaled down here to a few weights and a fixed rubric. Naming it this way sets expectations: discriminative models are smaller, cheaper, and deterministic in a way generative ones are not, which is precisely what lets them ride inside a tracer that has to give the same answer every time. The shorthand for the whole approach is small discriminative models: a set of decision heads and a judge, never a generator.

What is left is data, not model size. The corner head could ship because its labels were free: a real font's outline already says where the corners are (the next sections). But the labels that matter most, and are hardest to come by, are the judgment calls on soft scans of printed type where the edge is textured rather than clean, which no font outline and no synthetic degradation reproduces. So the current work is deliberately unglamorous: trace real images under a range of settings, keep the ones that come out right, and let each kept result become one row of the table the next head will fit. Until the selector's table is that full, the procedural rule stands in for it, and making that rule better is its own reward.
...
So the plan is to learn that intent from designers instead of guessing at it. The mechanism is already proven: the corner keep/drop head from the previous section is exactly this kind of model, shipping today on labels a font's own outline supplies for free, and designer cleanup is how a head learns the calls those clean outlines cannot teach, the keep-or-drop decisions on a rough scan that come down to taste. When you trace an image and then clean up the result by hand, your cleaned version is an example: every point you kept is a "keep," every point you removed is a "remove." A small model can learn from those examples to answer one narrow question. Given a point like this, in a context like this, would a designer keep it? It reads the same simple measurements the cleanup already computes, how far off the curve the point sits, how sharp the turn is, where it falls in the letter, and predicts the call.
...
There is, though, one source of correct answers that costs nothing and involves no scanning at all: a real font's own outline. Render an existing open-licensed font to a bitmap, trace it, and align the trace back to the font's true contours, and every decision the tracer made has a ground-truth label for free: the designer already placed the corners, the extrema, and the handles, and the outline records exactly where each one goes. img2bez-data does this across 27 masters spanning more than a dozen families and three scripts, turning each font into millions of labeled decisions with no human in the loop. This is not the smooth synthetic degradation that failed; it is real type, drawn by designers, used as its own answer key. It is the corpus that trained the corner head that now ships, and the pretraining set every later head will start from.

The font corpus gives structure at scale, but not taste: it records where Rubik's designer put a point, not which point a human would keep on a rough scan of type it has never seen. For that the tool also collects real data as a side effect of being used, in three streams that feed each other.
...
### 15. What the First Training Runs Found

The corpus described above stopped being a plan and became data: overnight runs now trace every glyph of 26 masters and label every gate decision against the font's own outline, about 2.4 million labeled corner decisions so far, plus smaller sets for the inflection, line, and merge gates. Training against it taught five things worth writing down, and not all of them are flattering.

First, scale did more than tuning. The first corner head, trained on one label pass, actually made real-world images worse: on a 104-image corpus of wild inputs (photos, scans, screenshots) it cost a hundredth of a point of quality. Retraining the identical model on 54 times the data flipped the sign, and the head now beats the hand-tuned rule on exactly the inputs the tool is named for. Nothing else changed. Same seven features, same 60 KB of trees.

Second, the learned boundaries transfer across designs. Held out entirely from training, Source Serif's corner decisions are predicted at 99.6 percent where the procedural rule manages 46.7; Merriweather and Inter behave the same way. The failures cluster too: hand-drawn and brushy designs (Shantell Sans, Recursive Casual) sit ten points behind everything else in every experiment. That tail is the concrete case for the style-conditioning stage, a small embedding that tells every head what kind of design it is looking at, because no amount of additional serif data will teach a model what a marker pen intends.

Third, the most important result was about measurement, not models. The per-candidate numbers said the procedural gate agreed with reference fonts only 46 percent of the time, which made the rule look broken. Clustering candidates into curvature zones, one decision per zone the way a designer sees it, put the same gate at 93 percent. The rule was never as bad as the metric claimed; the metric was counting each disputed zone ten times. The honest target is also richer than keep-or-drop: each zone in a real font is one of four designed structures, a sharp corner, a rounded corner drawn as two smooth points bracketing a tight arc, a single smooth point, or nothing. A quarter of all the labels collected are those bracketed round corners, which the binary framing simply discarded. The next head classifies the zone, not the candidate.

Fourth, a negative result: the inflection gate refused to be learned, and the reason matters. Only four percent of curvature crossings get an anchored point from a designer, and neither the rule nor a model can predict which from the features logged today (both score under 0.2 on the metric that matters for a rare class). The lesson is that a label has to match a decision's purpose. "Is a designer's point near here" is the right question for corners; for inflections the honest question is whether anchoring improves the fit, which is not something a reference lookup can answer. Not everything in the pipeline is a learning problem yet, and pretending otherwise would just launder thresholds through a model.

Fifth, evaluating at this scale is a bug-finder in its own right. Full-font runs hung the tracer on six glyphs out of tens of thousands, which turned out to be two independent non-termination bugs in the optimal curve fitter of the excellent geometry library underneath img2bez, both reproducible on its latest release with clean, well-formed input. The fitter that replaced it is internal, bounded by construction, and scored slightly better on the structural gates than the one it replaced. A deterministic, inspectable pipeline is what made that diagnosis a morning's work instead of a mystery; it is the same property that makes the learned heads safe to ship.

The bar for the next step is set in advance: the zone classifier replaces the procedural gate as the default only when it worsens zero glyphs on the structural gate and improves the wild corpus at the same time. Until a head clears that bar it stays opt-in, the rules stay the default, and every run that misses publishes its per-glyph failure table to the data repo instead of a threshold tweak to the tracer.

### 16. Early Days, and an Invitation
