...
Below is an interactive demo of img2bez compiled from Rust to WASM. Click **Trace** to vectorize a raster image and see the outline it produces. To try your own glyph, drop an image onto the app (or use the picker in the corner). This runs with the default settings, which can be tuned for better output depending on image quality and design style.
...
You can also use it in a font editor like [Runebender-Xilem](https://github.com/eliheuer/runebender-xilem) or [Runebender-Web](https://github.com/eliheuer/runebender-web); the simplest way is to go to [runebender.org](https://runebender.org) and use it directly in the browser: drag-&-drop an image into the edit view, then right-click it and choose **Trace Image**.
