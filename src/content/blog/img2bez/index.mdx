...
img2bez is most useful as a dependency inside something else: a desktop or web font editor, a font-production script, or an AI agent building sources on its own. So the API is built around one rule: **tracing a shape and placing it into a font are two different jobs, and most callers only want the first.**

Why keep them separate? `trace()` returns just the outline — drawn at a standard em scale, but not yet positioned in a font. `place()` does the positioning: it sets the baseline, the sidebearings, and the advance width, and only when you ask. Those are per-font choices, so you make them in your own coordinate system instead of unwinding mine. (The outline is plain x,y coordinates, so if your font uses a different em, you just scale it.)
