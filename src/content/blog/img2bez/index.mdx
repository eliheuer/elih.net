...
Raster-to-vector tracing is an old problem with lots of existing solutions, so a new tracer needs a justification. Mine is that the classical solutions, and newer solutions like differentiable tracing, optimizes the wrong objective for what I need and want for type (font) design, specifically piplelines and data engineering for AI automated type design.

I am working on a delusinally ambitious project in the space, font.garden. which has the goal of making open-source local AI a viable path for AI font development. In those pipelines letterforms keep showing up as raster images—scans of lettering, renders from image models, screenshots of old specimens. Image models in particular have gotten good at drawing letterforms, but they output pixels, and the gap between "a picture of the letter" and "a font source you can edit, interpolate, and compile" is tracing.
...
- A **straight run** is a maximal stretch whose chord deviation stays under max(0.15px, 0.005·ℓ) for chord length ℓ. The envelope grows linearly with ℓ on purpose: a true line's deviation is flat noise at any length, while an arc of radius r bows away from its chord by the sagitta ℓ²/8r—which grows with the square of ℓ—so a linear envelope separates lines from arcs at every scale with one pair of constants.
...
One amusing wrinkle: optimizing a cubic's two handle lengths independently stalls on arcs, because the loss forms a narrow diagonal valley of near-constant sum—many cubics trade tension between their two handles almost freely. Searching along the sum and the difference of the two lengths before the individual axes fixes it. This is the kind of thing the differentiable-rasterization papers handle with momentum; with two variables you can just look at the valley and aim along it.
