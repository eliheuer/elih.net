...
The reason is that a font source's *structure*, not just its silhouette, is what makes it usable. Constructing a font outline is a discipline, not unlike low-polygon 3D mesh construction in video games. Type design has strong drawing conventions that exist for mechanical reasons (editing cost, interpolation compatibility, rendering quality):
