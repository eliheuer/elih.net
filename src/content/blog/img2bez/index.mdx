...
The reason is that a font source's *structure*, not just its silhouette, is what makes it usable. Constructing a font outline is a discipline that needs to think about interpolation and movement of a structure, not unlike low-polygon 3D mesh construction in video games. Type design has strong drawing conventions that exist for mechanical reasons these reasons:
...
Headlessly it is one command, no editor in the loop:

```bash
img2bez --name germandbls --unicode 00DF \
  --master VirtuaGrotesk-Regular.ufo:regular.png \
  --master VirtuaGrotesk-Bold.ufo:bold.png \
  --fit-source ink --fit-y -256:768
```

It traces both images, reconciles them into one shared point structure, and writes the glyph into each master UFO with that weight's own advance width — interpolation-compatible, ready to compile.
