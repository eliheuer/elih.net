...
Raster-to-vector tracing is a well-solved problem, so a new tracer needs a reason to exist. Mine comes from the work I do: building fonts with AI, where image models generate letterforms and agents draw and edit the source files. The existing tracers, classical and differentiable, optimize for the wrong objective for that. The classical ones, like Potrace, start by thresholding the image: every pixel is pushed to pure black or pure white by a brightness cutoff, turning a soft gray picture into a hard two-color shape. They then walk the jagged staircase edge of that shape and fit smooth curves to it. The differentiable approach comes at it backwards. You can't read the right curves straight off a bitmap, but you can check a guess: draw a candidate outline and see whether its pixels match the target image. That is where a differentiable rasterizer like diffvg comes in. A plain rasterizer only draws a vector outline into pixels; a differentiable one also reports which way to move the control points to make the drawn result look more like the target. So tracing becomes a search: start from a rough guess, draw it, compare, nudge the control points to shrink the difference, and repeat until the rendering matches the image. It is the same kind of optimization that trains a neural network.

When you build fonts with AI, letterforms keep arriving as raster images: scans, type specimens, image-model generations. Vector-generating models like QuiverAI are improving, and some run locally, but most source material is still pixels, and turning a picture of a letter into a font you can edit, interpolate, and compile means tracing.

I use it for two things: tracing a background image inside a font editor, and running a fast first pass before an AI agent refines the source. The trace gets the outline about 90% of the way there for almost nothing, leaving the agent the last 10%. AI could do the whole job today, but handing the bulk to a deterministic step keeps the pipeline fast and cheap and avoids heavy local AI hardware.
...
One amusing wrinkle: tuning a cubic's two handle lengths independently stalls on arcs, because the loss runs along a narrow diagonal valley where the two handles trade tension between themselves almost freely. Searching along the valley (their sum and difference) before the individual handles fixes it. The differentiable-rasterization papers handle this with momentum; with only two variables, you can just look at the valley and aim along it.
