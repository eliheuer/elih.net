...
Headlessly it is one command per glyph, no editor in the loop. It reads the masters and their metrics from the font's designspace, so you hand it a folder with one image per master — each file named for its master — and the target band in the font's own terms:
