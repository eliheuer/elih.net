...
The profile matches image quality. It is auto-detected and can be forced with `--profile`:
...
1. **Reproduction:** render the traced outline back to a bitmap and measure how well it covers the input's pixels.
2. **Structure:** regularizers that encode the type-design rules directly, like the fraction of handles leaving horizontal or vertical and a penalty for the cluster of stray points that litters an over-traced terminal.
