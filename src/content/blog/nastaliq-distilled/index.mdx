...
longer-range effects. A production model must instead generalize beyond its
training examples. It should train on representative text and add cases where
its output differs from the teacher. A variable-length model could use a whole
word or line as context. Basic joining and mark-attachment rules, plus simpler
fallback forms, could handle invalid output.
