...
In a conventional font pipeline, shaping turns Unicode text into positioned
glyphs. Rendering draws those glyphs. The demo uses the NeuralType model and
engine to perform both steps entirely in the browser. The model has 1.36
million parameters. With 8-bit weights, the .ntf file is 1.38 MB. The
NeuralType engine is written in Rust, compiled to WASM, and built with
[Linebender](https://linebender.org/) libraries. The demo does not perform
...
I used [Gulzar](https://gulzarfont.org/) because it is the best OpenType font I
found under the SIL Open Font License (OFL). It is a contemporary Urdu
Nasta’liq typeface, not an exact match for these historical examples. [Noto
...
neither replaced OpenType. OpenType fonts also depend on shaping software, but
that support is already built into major platforms. Graphite and ACE require
...
first letter, I counted the distinct glyph IDs in the shaped output. Gulzar’s
per-letter counts add up to 344 across the 31 letters. The same test gives 150
for Amiri, a Naskh font with rich contextual behavior. Gulzar often represents
dots with separate glyphs, so its count includes both main forms and dots. This
measures contextual glyph inventory, not the number of complete letterforms.
...
receive vertical offsets as large as **1.38 em above the baseline**. The offsets
span more than one em vertically. The letters alternate between green and gray,
and the gray rule marks the baseline. Each red dot marks the origin of one
letter cluster. The red line connects these origins in writing order. The model
must learn the offset from one to the next.
...
glyph IDs with advances and offsets. The extraction code uses these values to
position the glyph outlines. The new model learns to copy this output. With the
shaping settings fixed, the pipeline is deterministic, so each input has one
target. That is a property of the teacher, not a limit of neural fonts.
...
for each cluster and uses the position of its first glyph as the cluster origin.
A record contains the cluster’s text context, composed shape, and origin. The
first cluster keeps its origin relative to the word origin. Each later cluster
keeps its offset from the previous cluster’s origin. These offsets are the red
steps in the figure above.
