...
font designed as a neural network could instead make new compositions based on a learned style. The whole line or even the whole page could adjust to fill space in a pleasing way. That is a goal for a future font designed from the ground up as a neural network, not a feature of this distilled Gulzar model.
...
intended for sustained reading. It is not a perfect match for the historical
examples, but it is the best available open-source option. I could not find
another usable OpenType Nasta’liq font under the OFL. [Noto
Nastaliq](https://notofonts.github.io/noto-docs/specimen/NotoNastaliqUrdu/) is a
poor-quality Nasta’liq design. [Awami
Nastaliq](https://software.sil.org/awami/resources/) is more capable, but it uses
Graphite rather than OpenType.

[Graphite](https://en.wikipedia.org/wiki/Graphite_%28smart_font_technology%29)
is a programmable font-shaping system developed by SIL. It stores fixed glyphs
and shaping rules in the font, and an application needs the Graphite engine to
run those rules. Awami does not support OpenType rendering.

NeuralType takes a different approach. Instead of requiring a font engineer to
describe the design with substitution and positioning rules, it learns shape
and placement from examples. NeuralType also requires its own engine today, but
a learned model could eventually respond to a whole line or page without
reducing the design to fixed glyphs and hand-written rules.
