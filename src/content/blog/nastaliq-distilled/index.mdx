...
shapes and positions with context. I started building it with
[`crates/neuraltype-distill`](https://github.com/eliheuer/post-opentype/tree/main/crates/neuraltype-distill).
In model distillation, the system a new model learns to imitate is called the
teacher. Here, Gulzar and HarfRust together serve as the teacher.

The tool shapes every possible one-, two-, and three-letter string, 30,783 in
all. Three-letter strings are the shortest that can contain initial, medial,
...
longer-range effects. An OpenType shaper applies its rules to any input. A
neural font must generalize beyond its training examples.

A production model should instead train on representative text and add cases
where its output differs from the teacher. A variable-length model could use a
whole word or line as context. Basic joining and mark-attachment rules, plus
simpler fallback forms, could handle invalid output.
...
The tool groups the glyphs into HarfBuzz clusters. Each cluster links one or
more input characters to the glyphs that represent them. One record per cluster
stores its text context, composed shape, and position. The first position is
measured from the word origin. Each later position is an offset from the
previous cluster. These offsets place the predicted shapes along Gulzar’s
cascade.
