...
corpus is still small. It covers every word up to three letters,
plus a first set of real words that includes the basmala. Words
outside the corpus render rough, and the demo font is a live
checkpoint, so the newest words improve as training runs.
...
Build-log status, 2026-08-01: the main run finished at contour IoU
0.91, and fine-tune passes on the extended corpus now stand at 0.93.
The corpus grew because of one word: الله. Gulzar draws this word
with a dedicated ligature, and no word in the first corpus was long
enough to trigger it. The extended corpus adds the basmala and other
real words, and the model is learning the ligature now. A final pass
with a decayed learning rate comes last. The same schedule took the
Kufic font from almost correct to exact.
...
4. Trace: **complete, simple version.** Sub-pixel marching squares
   with a bézier curve fit now. [img2bez](/blog/img2bez) is the
   planned quality pass for font-grade outlines.
