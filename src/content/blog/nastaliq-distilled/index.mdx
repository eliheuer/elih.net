...
In model distillation, the existing system a new model learns to imitate is
called the teacher. Here, Gulzar and HarfRust together serve as the teacher.

The tool shapes every possible one-, two-, and three-letter string, 30,783 in
all. Three-letter strings include cases where the middle letter joins on both
sides. It also shapes 24 real words from the bismillah, the surahs Al-Fatiha and
...
advances, and horizontal and vertical offsets. The
...
and shaping options fixed, the same input always produces the same shaped
output. That determinism belongs to the teacher used here, not to neural fonts
in general.

The tool groups the output into HarfBuzz clusters. Each cluster links one or
more input characters to the glyphs that represent them. The tool writes one
record for each cluster, storing its text context, composed shape, and position.
The first position is relative to the word origin. Each later position is an
offset from the previous cluster. These offsets place the predicted shapes
along Gulzar’s cascade.
