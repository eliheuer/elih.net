...
most. Nastaliq is a cursive, flowing style. Letters join or break by
the rules of the Arabic script, so a word is a chain of connected
runs. Within a run, each letter hangs below the letter before it, and
...
canvas, place the cursor, select, copy, and paste. The sample words
stay short for now because the training corpus stops at three-letter
words. Letters inside longer words sit in contexts the model has not
seen yet.
...
exercise. It shows that a font that contains a small neural network
is a viable alternative to OpenType. Once that is demonstrated, the
next step is new nastaliq fonts, designed to show what a NeuralType
font can do that an OpenType font cannot. Distillation also answers
...
That number breaks the metal-type box emulation concept and shows why a new format is needed.
...
Build-log status, afternoon of 2026-07-31: epoch 79 of 100, contour
IoU 0.90. The IoU still climbs. Thin vertical strokes form last, and the loss
...
The sheet below compares the teacher with the model checkpoint from
epoch 79. Gulzar is gray on top. The model is green below.

![Six corpus words rendered by Gulzar in gray above, and by the epoch-79 model in green below. The rows match closely.](./teacher-model.png)
