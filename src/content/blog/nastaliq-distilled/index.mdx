...
shapes and positions with context. To build that dataset, I wrote
[`crates/neuraltype-distill`](https://github.com/eliheuer/post-opentype/tree/main/crates/neuraltype-distill),
which extracts those shapes and positions from Gulzar. In model distillation,
the existing system that a new model learns to imitate is called the teacher.
Here, Gulzar and HarfRust together serve as the teacher.

The tool shapes every possible one, two, and three-letter string, 30,783 in
all because three-letter strings are the shortest that can contain initial, medial,
and final forms. The tool also shapes 24 longer words from the bismillah, the
surahs Al-Fatiha and Al-Ikhlas, and the word نستعليق.

Exhaustive coverage stops at three letters for now. Four-letter strings alone
would add 31⁴ = 923,521 inputs, about 30 times the complete one-to-three-letter
set. Five-letter strings would add another 28.6 million and still miss
longer-range effects. Instead, a production model should train on representative
text and add cases where its output differs from the teacher. Basic joining and
mark-attachment rules, plus simpler fallback forms, could handle invalid output.
Producing valid output for text the model has not seen is probably the hardest
technical problem facing neural fonts.
...
learn. That determinism belongs to this teacher, not to neural fonts in general.

The tool groups the result into HarfBuzz clusters, each linking one or more
input characters to their glyphs. It writes one record per cluster with its
text context, composed shape, and position. The first position is relative to
the word origin; later positions are offsets from the previous cluster. These
offsets place the predicted shapes along Gulzar’s cascade.

The corpus contains 91,390 cluster records and **1,150 distinct composed
shapes**. The letter خ uses 108 of them, shown below.

![All 108 shapes of خ in the dataset, in a grid.](./kha-shapes.png)

The corpus covers every short string but only 24 longer words. The model
therefore fails on some unseen words even when every letter is supported. In
the examples below, it breaks shapes and placement. Gulzar is gray; the model is
green.
