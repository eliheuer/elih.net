...
OpenType option I found. It is a contemporary Urdu Nasta’liq typeface, not an
...
Nastaliq](https://software.sil.org/awami/resources/) uses Graphite instead. I
found no other usable Nasta’liq font under the OFL. This shortage, relative to
the style’s historical and cultural importance, suggests that the usual
technical and economic approach is failing.
...
[HarfBuzz](https://harfbuzz.github.io/what-does-harfbuzz-do.html) combines its
own script logic with tables in the font. Graphite lets the font contain a more
complete shaping program. Awami uses this to avoid collisions and fit one
sloping cluster beneath another. The tradeoff is limited software support.

[DecoType’s](https://decotype.com/) Advanced Composition Engine (ACE) takes a
small set of functional shapes and combines them through an explicit model of
the script. It powered
...
spacing, and letter positions. ACE was proprietary and tied to a specialist
publishing workflow.

Graphite and ACE can do more than common OpenType fonts, but neither replaced
OpenType. Both need special fonts, a separate engine, and support from
applications. Neural fonts face the same problem. They need to offer a much
larger gain, such as learned contextual forms or whole-line composition, to
justify another engine. This distilled Gulzar model is only a test of that
idea.
