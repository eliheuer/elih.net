...
The measurements above show what the dataset must record: how Gulzar changes
shapes and positions in different contexts. I started building the dataset by shaping many strings with Gulzar and HarfRust and recording the results. Together, Gulzar and
HarfRust serve as the teacher: the existing system the model learns to imitate.

For each input string, HarfRust uses its Arabic shaping logic and Gulzar’s
OpenType Layout tables to select and position glyphs. It returns glyph IDs,
horizontal advances, and horizontal and vertical offsets. The
[extraction code](https://github.com/eliheuer/post-opentype/blob/main/crates/neuraltype-distill/src/main.rs#L306-L437)
records those positions and reads the corresponding outlines from Gulzar. With
the font and shaping options fixed, this pipeline is deterministic: the same
input string produces the same shaped output. That reflects the teacher used
for this experiment, not a requirement of neural fonts.
