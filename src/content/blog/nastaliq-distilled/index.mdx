...
the concept and was easy to dismiss as a toy tech demo. This post tests whether
a neural-network-based font can serve as a practical alternative to OpenType
fonts.
...
The demo uses the NeuralType model and engine to shape and render text entirely
in the browser. The model has 1.36 million parameters. With 8-bit weights, the
.ntf file is 1.38 MB. The NeuralType engine is written in Rust, compiled to
WASM, and built with [Linebender](https://linebender.org/) libraries. The demo
does not perform OpenType shaping at runtime. During training on an inexpensive
local GPU, the model learned to reproduce the shaped output produced by Gulzar and
...
An OpenType font could reproduce this exact composition with a ligature glyph
while preserving the underlying text. But the font would contain only one
finished design for one phrase. It would not learn how to compose other text. A
neural-network-based font could learn a style and use it to make new
compositions. A whole line or page could adjust to the available space. This is
a goal for a future font designed from the ground up as a neural network. The
distilled Gulzar model does not do this.
...
I used [Gulzar](https://gulzarfont.org/) because it is the best OFL-licensed
OpenType font I found. It is a contemporary Urdu Nasta’liq typeface, not an
exact match for these historical examples. [Noto
Nastaliq](https://notofonts.github.io/noto-docs/specimen/NotoNastaliqUrdu/) is
the only comparable OFL-licensed OpenType font I found, and, in my view, is
slightly worse. [Awami
Nastaliq](https://software.sil.org/awami/resources/) is also OFL-licensed, but
implements its shaping in Graphite rather than OpenType Layout. This shortage,
relative to the style’s historical and cultural importance, suggests that the
usual technical and economic approach is failing.

[Graphite](https://graphite.sil.org/) was developed by
[SIL International](https://www.sil.org/), a Christian nonprofit that develops
language software. With OpenType Layout, a shaping engine such as
[HarfBuzz](https://harfbuzz.github.io/what-does-harfbuzz-do.html) uses its own
script-specific logic to apply substitution and positioning tables stored in
the font. A Graphite font instead contains the complete rule set for its
shaping model. Awami uses these rules to avoid
collisions and fit one sloping cluster beneath another. The tradeoff is limited
software support.

[DecoType’s](https://decotype.com/) Advanced Composition Engine (ACE)
synthesizes contextual forms and alternatives through an explicit model of the
script. It powered
[Tasmeem](https://decotype.com/pdfs/Tasmeem_Manual.pdf), an InDesign extension
that kept text editable while giving designers control over word forms,
spacing, and letter positions. Tasmeem was a proprietary product built for a
specialist publishing workflow.

Graphite and ACE can express behavior beyond standard OpenType Layout, but
neither replaced OpenType. OpenType also depends on shaping software, but that
support is already built into major platforms. Graphite and ACE require
additional engines and application support. Neural fonts face the same adoption
problem, but take a different approach and can learn from these earlier
attempts. My long-term ambition is much larger. I think a format like this could
replace OpenType. It could support applications in marketing, branding,
advertising, and cultural production that make adopting a new format
economically viable. The gains must be large enough to justify the switch.
...
The figure below shows the word نستعليق shaped with Gulzar and HarfBuzz. Glyphs
reach **1.38 em above the baseline**. The word spans more than one em vertically.
The letters alternate between green and gray, and the gray rule marks the
baseline. Each red dot marks the origin of one letter cluster. The red line
connects these origins in writing order. The model must learn the offset from
one to the next.
...
The teacher is Gulzar shaped by HarfBuzz. For each input string, HarfBuzz uses
its Arabic shaping logic to apply Gulzar’s OpenType Layout tables. It returns
the selected glyphs and their positions. The new model learns to copy this
output. With the shaping settings fixed, the pipeline is deterministic, so
each input has one target. That is a property of the teacher, not a limit of
neural fonts.
...
and origin. The first cluster keeps its origin relative to the word origin.
Each later cluster keeps its offset from the previous cluster’s origin. These
offsets are the red steps in the figure above.
...
is measured from the previous cluster origin. To draw the shape, five layers
...
the teacher targets and combine the differences into one error score. The
...
by either shape. Identical shapes score 1.0. Shapes with no overlap score 0. The final
...
the same local context has different positions in different teacher examples,
the current training step averages those positions. The result may match none
of the examples.
...
and OpenType Layout substitution and positioning tables. A shaping engine such
as HarfBuzz applies those tables using its Arabic shaping model. The .ntf stores
a learned approximation of the resulting shapes and positions, but it also
requires the NeuralType engine. The comparison therefore measures font data,
not the complete software stack.
...
font designed for the format ready for ATypI 2026 in Sharjah. I will be there
to demo it, raise seed funding, or accept job offers at big tech companies to
continue this work.
