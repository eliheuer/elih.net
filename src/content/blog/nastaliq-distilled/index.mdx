...
The measurements above show what the training data needs to capture: how Gulzar
changes each letter’s shape and position in context. I then built the dataset by
shaping many input strings with Gulzar and HarfRust and saving each result as a
training target. In machine learning, the system that produces these targets is
called the teacher. Here, Gulzar and HarfRust form the teacher.

For each input string, HarfRust uses its Arabic shaping logic to apply Gulzar’s
...
uses these values to place the glyph outlines. Because the shaping settings do
not change, the same input string always produces the same result. This is a
property of the teacher, not a limit of neural fonts.
