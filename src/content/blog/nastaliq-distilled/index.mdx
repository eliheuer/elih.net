...
OFL license. The lack of fonts in this style, relative to its historical and
cultural significance, is, I think, an indication that something is wrong here.
...
[DecoType’s](https://decotype.com/) Advanced Composition Engine (ACE) takes a
different approach. An ACE typeface contains a small set of functional shapes
rather than every finished glyph. The engine combines these parts according to
an explicit model of the script. DecoType used ACE to build its own
[Nasta’liq typeface](https://decotype.com/pdfs/TDCtdc2010_Nastaliq_v05a.pdf),
which can form different Arabic-script languages and place their marks from the
same basic set of parts.

ACE reached designers through
[Tasmeem](https://decotype.com/pdfs/Tasmeem_Manual.pdf), an extension for the
Middle Eastern edition of Adobe InDesign. Tasmeem kept the Unicode text
searchable and editable while letting the user choose alternate word shapes,
adjust Arabic spacing, and move letters, dots, and vowel marks. It addressed
many of the problems shown in the comparisons above without reducing a passage
to one ligature glyph.

This power came in a proprietary system built for a specialist publishing
workflow. ACE typefaces require the ACE engine, and Tasmeem did not become a
general font system supported by browsers and operating systems. Like Graphite,
it shows that a better composition model can exceed OpenType while still
struggling to gain broad software support.

Graphite and ACE encode behavior supplied by the font maker. Neural fonts also
require a new engine, but they learn shape and placement from examples. A
neural font could generate forms for the exact context, compose a line to fit a
given space, or reproduce lettering styles that would be impractical to encode
as glyphs and rules. These abilities could support commercially useful display
type, branding, and advertising, as well as scripts such as Nasta’liq. That
range of uses could give industry a reason to support the format.

This distilled Gulzar model does not have these abilities. It is a first test of
the underlying approach. New fonts need to be designed for the format. Before
that, we need font-editing software built for training neural-network-based
fonts. I plan to have a prototype of this software and the first Nasta’liq font
designed for the format ready for ATypI 2026 in Sharjah. I will be there to demo
it, raise seed funding, or accept job offers at big tech companies to continue
this work.
...
shape only, not position. It does not reflect a known placement problem: when
Gulzar places the same local context differently in different words, the
current training step averages those positions. The result may match none of
the examples.
...
The two files do not perform exactly the same job. Gulzar stores outlines and
OpenType rules that a separate shaping engine must interpret. The .ntf stores a
learned approximation of the shaped result, but it also requires the NeuralType
engine. The comparison therefore measures font data, not the complete software
stack.
...
targets. When Gulzar places the same shape differently in different words, the
model should choose among those positions using the rest of the line. It should
not average them into a position that may occur nowhere in the source.
