...
The current model, Virtua-12M-v0.1, beats the baseline. The evidence
is a controlled comparison of two training runs. The first trained
only on the graded corpus (50 approved Bold pairs, up from 39): it
tied the baseline. The second pretrained on 29,000 pairs traced from
Google Fonts, then finetuned on the same graded corpus: it beats the
baseline on all ten held-out glyphs, on every metric (seed-averaged).
Cleaning the data kept the tie; other designers' letterforms broke
it.
...
training run each, so the error bars cover sampling variance, not
