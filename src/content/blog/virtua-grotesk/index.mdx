...
**1. Normalize exactly.** Divide Virtua Grotesk's 96-unit
...
(The formats do not force this. The old rule that ascender minus
descender must equal the em was dropped from the OpenType spec, and the
tables accept any values. The pressure comes from the design's own
proportions and from the arithmetic at render time, below.)
...
The weights are not free either: OpenType stores a normalized axis
position as an F2Dot14, a fraction over 2¹⁴, so every instance a
variable font can express sits at a binary weight. The format cannot
write 1/3. The dyadic grid isn't a lattice the operations tolerate;
it's the lattice they generate. Start from any grid, run the arithmetic, and the points
...
only by decree, one prescribed rounding at a time; exact values meet it
for free.

The machine's three constraints are half the answer; the other half is
where this parts ways with Replica. Replica's grid is one flat layer,
every legal point equally legal, so it can say only one thing about a point, and
...
this the 2-adic valuation, written ν₂; hardware calls it
count-trailing-zeros, one instruction. The hierarchy isn't drawn on top of the coordinates; it *is*
...
The 2-grid corrections are the first to dissolve into antialiasing, as
they should be: 2/1024 of an em is under a pixel at text sizes, and the
eye's touches vanish before the structure does. At 16 pixels, the web's
...
whichever renderer draws it, in whichever direction it prefers.
...
So the hierarchy's role assignments are not bookkeeping. Putting the
vertical metrics on the 64-grid puts the values the whole page shares
on the level that never rounds; structure holds to every even size; the
corrections are spent last, where antialiasing absorbs them. The level
of a coordinate is how far down the scaling ladder it survives, and a
1000 em has no ladder to survive on.
...
![A horizontal ruler drawn with designbot with ticks every 1/64 of a pixel and a taller tick at the nine-pixel boundary. A green point sits exactly on the tall tick, labeled 9.000: the 1024-em x-height at 16 pixels. A red point sits between ticks, labeled 8.992: the 1000-em x-height.](./fig-ruler.png)
...
power-of-two em, unique up to one free choice, the exponent. The eye fixes it: four decades of
