...
The encoding makes the task local. The two masters are written into
one sequence, point by point: a Regular point, then its Bold value,
then the next Regular point. The model predicts each Bold value from
the Regular point beside it, as a small offset. The Regular points
are fixed. The model fills in only the Bold values, so the structure
of the glyph cannot break.
