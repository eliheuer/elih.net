...
write, like the words of a language. Here that set holds three kinds: drawing
verbs (`MOVE`, `LINE`, `CURVE`, `CLOSE`); conditioning tokens that name the glyph
and set its weight (the prompt, what to draw); and one token for each legal
...
into a UFO outline: a real, editable glyph. The round trip is lossless: the
coordinate tokens are exactly the grid's legal positions, and the designer drew
every point on the grid, so each coordinate already equals one token, with no
rounding either way. Tokenizing loses precision only when a point falls between
two positions and gets snapped to the nearer one, and on a grid-native font
nothing does, because the snapping happened by hand, at design time. (What makes
it exact is committing to the grid, not how coarse it is: a 1-unit grid would
round-trip just as cleanly, only with more tokens.)
...
The model is deliberately small: a 12M-parameter decoder-only transformer that
trains from scratch on a single machine. It has two implementations,
[MLX](https://github.com/ml-explore/mlx) for an M4 Mac and PyTorch for a Linux
box with a gaming GPU. No cloud, no GPU cluster: an overnight run is
...
an M4 Pro or gaming GPU. In fact, the honest origin of this whole design
