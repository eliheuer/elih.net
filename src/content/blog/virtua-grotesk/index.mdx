...
<p class="font-mono text-sm" style="color: #ff4535">DRAFT: this post is a draft and still being edited every day.</p>
...
but the system is easier to draw than to tabulate. The two dimension
sheets below are measured directly from the Regular sources, every
dimension line anchored to real outline coordinates, so unlike a table
they cannot drift out of date. First the whole Latin system on four
glyphs, then the lowercase up close:

![Dimension sheet drawn with designbot: the word OHno set in Virtua Grotesk Regular on the design grid, with blue tags for cap and ascender height 768 (512 plus 256), x-height 576 (512 plus 64), baseline, descender minus 256, and the 16-unit overshoots, and green dimensions measuring the O curves at 108 and 100, the H stem at 104 and bar at 96, the n stem at 96, and the o curve at 100.](./fig-system-ohno.png)

![Dimension sheet drawn with designbot: the word no set large in Virtua Grotesk Regular, with the n stem measured at 96, the o curves at 100 and 92, a 16-unit chamfer called out at the n's foot, x-height and overshoot lines tagged in blue, and hatched sidebearings with ink widths dimensioned below the baseline.](./fig-system-no.png)

(The Bold and Arabic stroke values, and the rest of the spec, are in
`DESIGN.md`.)
...
The current model, Virtua-12M-0.7, now statistically ties the dumb
baseline of section 06 on held-out glyphs: 33.9 font units of error per coordinate
against the baseline's 33.3. A tie with a baseline is not a victory lap;
it is the floor the recipe demands, reached. The review sheet shows what
the number looks like: six glyphs whose Bolds the model never saw, human
Regular in, model Bold out, the human Bold underneath for reference.

![Held-out review sheet drawn with designbot: for the glyphs K, E, M, n, b, c and a, three rows show the human-drawn Regular input in green, the Bold drawn by Virtua-12M-0.7 in red, and the human-drawn Bold reference in gray. The a column's reference cell reads "never boldened": the Bold master never had a real a.](./fig-model-review.png)
...
a second), but it is a draft where there was nothing, and it now sits in
the Bold sources as a red-marked draft awaiting the human 2-unit pass.

![The letter a twice on the design grid, drawn with designbot in the Runebender point language: the hand-drawn Regular in green, and beside it the Bold in red, the first real Bold a in the design, drawn by Virtua-12M-0.7 from the Regular input.](./fig-model-bolden-a.png)
...
and the Regular-to-Bold deltas of section 07 are all drawn from 2, 4, 8,
16, 32, 64, 128 and short sums of them, a set closed under halving and
doubling, so making a glyph bolder moves its measurements within the same
small set. Boldening collapses from predicting arbitrary integers to
choosing among a few dozen familiar offsets.
