...
there, close the path), wrapped in layers that substitute and reposition glyphs
and interpolate new weights.

In 1964 Karl
Gerstner published Designing Programmes, whose first essay is titled "Programme
as Typeface" and whose thesis was "instead of solutions for problems, programmes
for solutions": design the system that generates the forms, not the forms one at
a time. He built one, too. Gerstner-Programm, begun that same year at GGK Basel
and drawn by Christian Mengelt for the Berthold foundry, unified a whole type
family by strict internal regularity rather than the inherited conventions of
how the letters were supposed to look. Donald Knuth then carried the idea to its
limit in Metafont, which turned letters into literal parametric code, and decades
later Jürg Lehni and Peter Biľak collected the argument under the title [Typeface
as Programme](https://www.typotheque.com/articles/typeface-as-programme).

What most of them wanted from the program was generative letterforms, strokes as
functions. I want something narrower and stranger: not to program the letters,
but to draw them regularly enough that a neural network can learn them.

Most fonts don't really think about any of this. Their coordinates land wherever
...
This sits inside a longer lineage: Helvetica's neo-grotesk neutrality, Wim
Crouwel's New Alphabet (1967) building every letter on a strict grid, Vignelli's
conviction that discipline and restraint are what make design intelligent, the
Swiss idea that order is a kind of honesty, work laid out in service of the
reader. I love that tradition. But
...
Modernist design was a reaction against design as personal performance, and a
desire to live in harmony with a rapidly advancing technological society. It
wanted anonymous, systematic form, the designer serving the
message rather than signing it. The odd thing is the timing: the backlash
against modernist type arrived just as the personal computer did, the very tool
that could finally deliver it. A rule-based, reproducible typography is a
program, and before the machine it could only be prototyped by hand, clumsily.
The revolt was not against the idea but against that rough draft, thrown out just
as the tool to finish it arrived.

A hundred years later, we are living through a very similar moment. The excess it
opposed is back as generated slop, endless type with no decision behind it. My argument is not nostalgia: the grid was always a way of
thinking clearly, and now it is also what makes a font legible to a machine.
...
Andrej Karpathy likes to call a large
language model [a zip file of the
internet](https://www.youtube.com/watch?v=zjkBMFhNj_g): billions of parameters
that compress terabytes of text into a lossy gestalt, keeping the patterns and
discarding the surface. A trained model is a compression of its training data.

Type designers already build compressors. A variable font is one: instead of
shipping a separate file for every weight, you ship a base outline plus a set of
interpolation deltas, the differences that bend it into every other weight, and
the font reconstructs them on demand. A neural network trained on a
typeface is that idea taken further, a lossy, learned compression of the design,
except it interpolates through a space it discovered for itself rather than one a
designer drew by hand. And a compressor is only as good as the regularity of what
it feeds on.

Everything Norm did for design discipline turns out to be exactly what a neural
network wants from its training data:
...
Andrej Karpathy makes the sharpest version of this point about arithmetic.
Language models are notoriously bad at it, and much of the reason is
[tokenization](https://www.youtube.com/watch?v=zduSFxRajkE): numbers get split
into tokens inconsistently, so `127` is a single token while `677` is two, and
the model has to do math on a representation that changes shape for no reason.
There is no canonical way to write a number down. Virtua's grid is the opposite.
Every coordinate lands on the same power-of-two lattice and tokenizes exactly one
way, a single canonical quantization with no arbitrariness for the model to
fight. The discipline a designer imposed for the eye hands the machine the clean,
regular number line that tokenizers never give it.
...
at scripts that do not work that way. Arabic in its manuscript forms is not a kit
of reusable shapes but a continuous, context-dependent stroke, and the current
workaround is thousands of glyphs and substitution rules approximating what a
hand does in a single motion. A generative font, a small neural network that has
compressed the typeface and draws each glyph in context instead of looking it up,
handles that natively. I would not be surprised if the first ones are built on a
grid like this one, because the grid is what makes the drawing learnable in the
first place.
