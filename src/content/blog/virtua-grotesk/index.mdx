...
Virtua Grotesk is a typeface drawn on two nested grids: an 8-unit grid
and a 2-unit subgrid. I call this a **self-labeling grid system**, and this concept makes the
font both a type design project and a machine-learning and
data-engineering project. If you are reading this on [elih.net](/), Virtua Grotesk
is the typeface you are reading now.

Every measurement defaults to a power-of-two grid. The system prefers
powers of two, but not at the expense of neo-modernist letterforms or
optical corrections. Section 03 explains how each deviation from a pure power-of-two measurement is ranked based on how elegantly it can be created from sums of powers of two, and this also becomes a free training label and is part of the overall grid system.
[A Python script](https://github.com/eliheuer/virtua-grotesk/blob/main/scripts/grid_qa.py)
grades every measurement against the system. An AI agent harness fixes
what the QA tool flags in a report, fix, render loop.

The model will soon be on Hugging Face as Virtua-12M-v0.1. It is an
open-weight model that anyone can run locally and fine-tune. The font
sources and harness are already on
...
Neural networks might change that. A model is not a program. It
interpolates and tolerates nuance where a rule is brittle. It also
answers a modernist ideal that the rigid version betrayed: technology
people can live with, not technology that extinguishes human dignity.
The old impulse is worth another look.

The problem is data. A neural network only knows what it is trained on.
Until recently, most fonts were not built as training data or for agent
editing through a harness. Their coordinates land wherever the
designer put them. Their contours are as individual as handwriting.
That is fine for a rasterizer. It is a problem for an AI production
pipeline: the network trains on the sources, the linter checks generated
glyphs, and the agent edits the files through a harness. Today's font AI
trains on exactly this accidental data: scraped fonts and Google Fonts,
...
grid-qa` grades every glyph in both masters for compliance. Every point
...
with mark colors as the human's control. The goal is to finish Virtua Grotesk
