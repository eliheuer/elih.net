...
Two masters is more data than it looks. Every interpolation between
them is a real instance of the family; Multiple Master fonts have
worked this way since 1991. The factory picks a weight between Regular
and Bold, interpolates, snaps to the grid, and labels the glyph with
its exact weight.

An interpolation lands between grid points, so is it worse data than a
master? No, and the outline sheet below shows why. Here is the real
...
![Three large lowercase n outlines set tightly on one continuous 8-unit grid: the n at Regular, at the 1/2 interpolation between Regular and Bold, and at Bold. All on-curve and off-curve points are marked and colored by grid level, green on the 8-unit structure grid and red off 8 on the 2-unit correction grid. Green stem dimensions read 96, 144, 192; gray coordinate labels give the arch apex at (328,592), (380,592), (432,592), the same y with the x interpolating; blue tags mark the x-height at 576 and the baseline at 0. Every interpolated point lands on the grid, so the intermediate weight is a clean, grid-native glyph.](./fig-interp-outlines.png)

The reason is arithmetic. Interpolation is halving, and a power-of-two
grid is closed under halving: the midpoint of two grid points is on
the grid, or one level finer down the same ladder, exactly. A
1000-unit em is not closed: halving a 10-unit interval gives 5, then
2.5, then 1.25, values off any clean level that must be rounded back,
a small error in every sample. This is why a font built to be training
data belongs on 1024, not 1000: one em makes noise-free data, the
other cannot. Section 09 proves the general case.
...
That pretraining data has its own held-out test, with one positive
result and one negative. One percent of the OFL pairs, 290 of 29,000,
are held out. On them the pretrained model beats the baseline, so the
...
styles it has seen; a font-level split is next.) The finetune then
costs that generality. After specializing on Virtua, the model
collapses on OFL, from beating the baseline on 74 percent of pairs to
9. This is catastrophic forgetting, the price of finetuning a small
model without mixing the old data back in; that mix is queued for
v0.9. Virtua-12M-v0.1 is a Virtua specialist, not a general tool:
...
is the review surface. A harness runs the loop against `DESIGN.md`,
with mark colors as the human's control. The goal is to finish Virtua
Grotesk and ship it to Google Fonts.
