...
grid. The grid a point lands on records what the point is for, so
the font's source files are labeled training data. I call this a
**self-labeling grid system**. It makes the typeface a
machine-learning and data-engineering experiment as much as a
visual-design one. A small neural network is now learning to draw
it. If you are reading this on [elih.net](/), you are reading
Virtua Grotesk right now.

Everything is built on powers-of-two: the em is 1024 = 2^10 units,
stems are 96, chamfers 16, sidebearings 48 (32 + 16), and so on. The model will soon be on
