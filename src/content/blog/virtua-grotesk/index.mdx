...
The lineage has a font branch too, worth naming to place this work on
a map. [SVG-VAE](https://arxiv.org/abs/1904.02632) (Lopes et al.,
2019) learned glyphs as SVG command sequences from a corpus of tens of
thousands of fonts; [DeepSVG](https://arxiv.org/abs/2007.11301)
(Carlier et al., 2020) built a hierarchical transformer for vector
graphics; [DeepVecFont](https://arxiv.org/abs/2110.06688) (Wang and
Lian, 2021) generates whole vector fonts with dual raster-and-sequence
supervision. All three learn from large heterogeneous corpora and
spend much of their capacity absorbing that heterogeneity: coordinates
that land wherever thousands of different hands put them. This project
inverts the ratio. Design one corpus until it is nearly homogeneous,
then ask how small the model can get.
...
do have. Beating it means beating the best non-learning tool that can
even enter the contest.
...
held-out glyphs, on every metric. Data cleaning alone kept the tie;
...
data demonstrably beats both a strong baseline and a bigger helping of
parameters; whether that holds three orders of magnitude up, against
...
29,000. On those, the pretrained model beats the mean-delta baseline
...
When I shared this draft, the most common pushback was why powers-of-two, so I added this additional section, where I try to mathematically
