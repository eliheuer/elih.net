...
![The word Grid set in Virtua Grotesk as a dimension sheet: red glyphs on the 16-unit design grid with every on-curve and off-curve point marked, green points on the 8-unit machine grid and red points off 8 on the 2-unit human grid, blue tags for cap height 768, x-height 576, baseline and the overshoots, and hatched sidebearings with ink widths dimensioned below the baseline.](./share-card.png)
...
its grid level. Green points sit on the 8-unit machine grid; red
points sit off 8 on the 2-unit human grid, the optical corrections.
...
![Dimension sheet drawn with designbot: the word OHno set in Virtua Grotesk Regular on the design grid, every on-curve and off-curve point marked, green points on the 8-unit machine grid and red points off 8 on the 2-unit human grid, with blue tags for cap height 768, x-height 576, baseline, descender minus 256 and the overshoots, and green dimensions measuring the O curves at 108 and 100, the H stem at 104 and bar at 96, the n stem at 96, and the o curve at 100.](./fig-system-ohno.png)
...
![Dimension sheet drawn with designbot: the word no twice, Regular then Bold, with full point annotation, green points on the machine grid and red optical corrections, stems measured at 96 and 192, curve strokes at 100 and 196, and a red callout naming an off-8 point an optical correction.](./fig-system-weights.png)

![Dimension sheet drawn with designbot: alef, beh and medial heh reading right to left with full point annotation. The alef sits entirely on the 8-unit machine grid at stroke 96; the beh's calligraphic tail is dense with red 2-grid optical corrections, its boat stroke measures 72 and its dot 160 across.](./fig-system-arabic.png)
...
![A cubic arch over the two-level grid, drawn with designbot: every point lands on the 8-unit machine grid in green, except the apex and its two handles, pulled 4 units up onto the 2-unit grid, marked in red with a plus-4 dimension and a callout labeling the move an optical correction.](./fig-optical-correction.png)
...
detectable and snappable, wrong stem weights are measurable. Generation
quality becomes measurable and an eval loop can grind at it overnight.

A note on stating that precisely, because an earlier draft said the
stems were "power-of-two sums" and Simon Cozens correctly pointed out
that every integer is one; that is what binary means. The property with
content is not decomposability but *trailing zeros*: 96 is a multiple
of 32 and 95 is a multiple of nothing, so 96 sits five rungs up the
ladder of section 03 and 95 sits on the floor. And the stroke weights
that look arbitrary are the two-tier grid in action: the lowercase
curve strokes are 100 and 92, which is the 96 stem plus and minus 4,
the hand's optical compensation for curves, recorded on the fine grid.
The system is machine values with many trailing zeros, corrected by the
eye in units of 2 and 4. That is the multi-layer semantic grid, and it,
not any single number, is the innovation this post is about: which layer
a value sits on *is its meaning*. The n's stem is 96 because structure
belongs to the machine's grid; the o's curve is 100 because a round
stroke needs the eye's +4, and the 4 lives on the layer reserved for the
hand. The powers of two are simply the base in which such a ladder of
layers runs deepest, and, separately, the numbers I find beautiful.

And the claim that the layers are learnable is testable, because the
model's vocabulary enforces only the bottom layer: every coordinate
token is a 2-grid position, all of them equally available, and nothing
in the encoding mentions the 8-grid at all. A model emitting random
legal coordinates would put about 94 percent of its points off the
machine grid. Measured on held-out glyphs whose Bolds it never saw,
Virtua-12M-0.8 puts 68 percent of its points *exactly on* the 8-grid,
against the hand's 85 percent in the sources, and when it does leave
the machine grid it prefers the hand's own on-4 values to arbitrary
positions. On the E, a glyph with no optical corrections at all, it
stays on the machine grid at every single point. Nobody labeled any of
this. The only supervision was outlines that obey the semantic grid,
and the model recovered the layers from the geometry alone.

![The whole argument in one dimension sheet, drawn with designbot. Left: the word no with its points colored by grid layer, the n stem dimensioned 96 in green as machine structure, the o curves dimensioned 100 and 92 in red as the hand's corrections. Top right: 96 and 100 written in binary with their trailing zeros bracketed, under the note that every integer is a sum of powers of two and the meaning is the trailing zeros; 100 equals 96 plus 4, the curve's optical correction. Bottom right: bars showing the share of points landing exactly on the machine 8-grid in held-out model output: chance 6 percent, Virtua-12M-0.8 68 percent, the hand 85 percent, with the note that nobody labeled the layers; the model found them.](./fig-semantic-grid.png)
...
coarse and kept. As far as the model is concerned, the base is a matter of
taste; section 10 makes the case that the rest of the pipeline disagrees.
...
the machine's structural skeleton, 2 for the human hand, so which level
...
116 ends in two, the 4-grid, off the machine's 8, which is exactly
what marks it as the hand's. Mathematicians call this the 2-adic
...
![Five rows of eleven-bit binary numbers drawn with designbot: the em 1024, cap height 768, x-height 576, the 96-unit stem, and the hand-corrected 116, each with its run of trailing zeros bracketed and its grid level read off as the count, the 116 row in red.](./fig-bits.png)
...
block of font units: the machine grid reappears, uninvited, inside the
