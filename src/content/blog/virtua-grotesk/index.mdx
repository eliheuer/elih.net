...
The test setup. Ten glyphs were set aside before training; their
Bolds were never shown to either run. Every model number in the table
...
This is a small test: ten glyphs, one font, one training run each.
The numbers will move as the work grows. The output is a draft, not a
finished Bold; a person still corrects it on the 2-unit grid. The
...
Two masters sounds like no data at all. But every interpolation
between them is a real instance of the family; Adobe's Multiple
Master fonts have worked this way since 1991. The factory picks a
weight between Regular and Bold, interpolates, and snaps to the grid.
Each new glyph carries its exact weight as a label.

A fair worry: the masters sit exactly on the grid, but an
interpolation lands between grid points. Is it worse data? No, and the
reason is the case for the em. Interpolation is halving, and a
power-of-two grid is closed under halving: split the gap between two
grid points and the result is on the grid, or one level finer down the
same nested ladder, exactly, with no rounding. The factory
manufactures exact data forever. A 1000-unit em has no such closure:
halve a 10-unit interval and you get 5, then 2.5, then 1.25, values
that belong to no clean level and have to be rounded back, a small
error in every sample. This is the concrete form of the section 09
proof, and it is the reason to build a font meant to be training data
on 1024 rather than 1000: the one em manufactures noise-free data, the
other cannot.

The one thing interpolation cannot invent is a new optical correction;
it blends the masters' corrections in a straight line, and optics are
not linear in weight. So it teaches structure exactly and approximates
the fine corrections. The model learns optical taste from the two real
masters and the graded corpus, not from the augmentation.

Add contour-start rotation, since any cyclic rotation of a closed
contour draws the same shape, and two masters become an endless
stream of distinct batches. Enough to keep the model from
memorizing.

That is volume, not variety. Every batch is still one design. Other
hands take more source families, and that is where
[img2bez](/blog/img2bez) comes in: a Rust autotracer that traces any
raster, a rendered font, a scan, an AI-drawn glyph, into outlines
snapped to the 2-unit grid.
[img2ufo](https://github.com/eliheuer/img2ufo) assembles them into a
full UFO source. Together they turn a type specimen image into a
grid-native font in about a minute, already in the same coordinate
language. The plan is not to bulk-convert Google Fonts. That library
is large and uneven, and running all of it through the grid-ifier
would only move the quality problem onto the grid. The plan is to
engineer good data: bring a strong family onto the grid, grade it,
keep what passes. The grid-ifier makes each conversion cheap, so the
work becomes curation, a growing corpus of high-quality grid-native
families, every one OFL, with a provenance manifest you could audit
glyph by glyph. It also answers the charge that the grid is one
foundry's private convention: any OFL family can be brought onto it.

The pretraining corpus supports its own test, with a positive result
and a negative one. One percent of the OFL pairs, 290 of 29,000, are
held out. On them the pretrained model beats the baseline, so the
skill is not Virtua-specific: it draws Bolds for glyphs it has never
seen. (The held-out unit is the glyph, not the whole font, so this is
new glyphs from styles it has seen; a font-level split is next.) The
negative result is what the finetune costs. After specializing on
Virtua, the same model collapses on OFL: from beating the baseline on
74 percent of pairs to 9 percent. This is catastrophic forgetting,
the known price of finetuning a small model without mixing the old
data back in. That mix is queued for v0.9. Virtua-12M-v0.1 is a
Virtua specialist, not a general tool:
...
The pieces converge into one system. [img2bez](/blog/img2bez) traces
references into grid-snapped sources. The model completes outlines,
draws Bolds, and soon repairs them.
[designbot](https://github.com/eliheuer/designbot) renders the proofs
and every figure in this post. [Runebender](https://runebender.org)
is the review surface. An agentic harness runs the loop against
`DESIGN.md`, with mark colors as the human's control channel. The
goal is concrete: **finish Virtua Grotesk and ship it to Google
Fonts**, the model doing the tedious production work and a human doing
