...
down to the em square itself: 1024 (2^10) rather than the usual 1000. That is
...
A glyph is already a kind of sentence: an ordered list of drawing commands, read
start to finish. That is exactly what a transformer is built to predict, a
sequence of symbols where each one follows from the ones before. So instead of a
picture of a letter or a loose set of points, I give the model each glyph as the
...
A transformer works from a fixed *vocabulary*: the complete set of distinct
tokens it is allowed to read and write, like the words available in a language.
Every glyph sequence is spelled out of this one set. It holds tokens of three
kinds: drawing verbs (`MOVE`, `LINE`, `CURVE`, `CLOSE`); conditioning
tokens that name the glyph and set its weight (the prompt, what to draw); and
one token for each legal position on the grid. Here is the numeral **2** in the
Regular weight:
...
The two grid levels then sort those coordinates by role. Structure sits on the
8-unit grid, so its coordinate tokens turn up constantly; the positions in
between, on the 2-grid but off the 8, appear only where the eye asked for an
optical correction, so they stay rare. The 2-grid decides how many coordinate
tokens exist; the 8-grid decides which of them are common. And a coordinate off
the 8-grid is, by construction, a correction, so in the token stream the
corrections are simply the rare, off-8 tokens: the same self-labeling the grid
gives a human reader, now visible to the model as low-frequency, high-information
positions.
...
generate, you give it the opening tokens (the name and weight you want), let it
finish the sequence, and run the codec backward into a UFO outline: a real,
editable glyph. The round trip is lossless, and here is why. The coordinate
tokens are exactly the grid's legal positions, and the designer drew every point
on the grid, so each coordinate already equals one token exactly: no rounding to
encode it, none to decode it. The exactness comes from the design committing to
the grid, not from how coarse the grid is; a 1-unit grid would round-trip just as
cleanly, it would only need more tokens. Tokenizing loses precision only when a
point falls between two legal positions and gets snapped to the nearer one, and
on a grid-native font nothing falls between, because that snapping already
happened, by hand, at design time.
...
microgpt's cousin in another alphabet: glyphs in, glyphs out, forty minutes on
an M4 Pro. In fact, the honest origin of this whole design
