...
interpolation lands between grid points. Is it worse data? No. It is
snapped before it becomes data, so every training glyph is fully on
the grid, as grid-native as a master. The weights are binary
fractions, and the grid is closed under them (section 09), so the
blend lands back on the grid with little rounding. The one thing
interpolation cannot invent is a new optical correction; it blends the
masters' corrections in a straight line, and optics are not linear in
weight. So it teaches structure exactly and approximates the fine
corrections. That is the right split: the model learns optical taste
from the two real masters and the graded corpus, not from the
augmentation.
