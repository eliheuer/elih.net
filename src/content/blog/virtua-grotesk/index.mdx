...
![Glyph completion of the figure 2, drawn with designbot: gray reference, the green 40% given to the model, and the model's red completion on the powers-of-two grid. Preliminary model output, refreshed each training run.](./fig-complete-two.png)
...
![Regular-to-Bold weight transfer on a held-out B, drawn with designbot: the green Regular input, the model's red predicted Bold, and the gray actual Bold on the powers-of-two grid. Preliminary model output, refreshed each training run.](./fig-bolden-b.png)
