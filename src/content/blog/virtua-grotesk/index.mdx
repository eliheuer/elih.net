...
aesthetic and visual-design experiment. The same grid that gives the font its
aesthetic makes its source files unusually good training data, and a
small neural network is now learning to draw it. If you are reading
this on [elih.net](/), Virtua Grotesk is the typeface you are reading
right now.

The grid has two nested tiers: structure on the 8-unit grid, optical
correction on the 2-unit grid. Everything is ruled in powers-of-two,
and every dimension reaches for an elegant binary number: 1024 = 2^10
units per em, stems of 96, chamfers of 16, sidebearings of 48
(32 + 16), and so on. The model will soon be on Hugging Face as an
open-weight release that anyone can use and fine-tune on their own
fonts. The font sources are on
