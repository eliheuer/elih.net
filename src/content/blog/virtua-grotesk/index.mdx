...
The font sources are on
[GitHub](https://github.com/eliheuer/virtua-grotesk),
...
<li><a href="#10-the-mathematics-of-2ⁿ"><span class="n">10</span>The Mathematics of 2ⁿ</a></li>
</ol>
</nav>
...
<p>—Donald Knuth, [The TeXbook](https://www-cs-faculty.stanford.edu/~knuth/abcde.html), 1984.</p>
</div>

When I shared this draft, the most common pushback was also the fairest one:
why powers of two? Any grid shrinks the coordinate vocabulary. Replica's
decimal grid would tokenize just as cleanly, so if the argument is
consistency, the base looks like superstition. This section is my best
attempt at an answer, and it starts with a concession: the transformer
mostly does not care. A 10-unit grid on a 1000-unit em gives a small,
consistent vocabulary too, and I can find no evidence that a network prefers
binary-spaced bins to decimal ones. What the field shows is only habit:
[PolyGen](https://arxiv.org/abs/2002.10880) quantizes mesh vertices into 256
bins and [DeepSVG](https://arxiv.org/abs/2007.11301) does the same for
outline coordinates, because 2⁸ is what an engineer reaches for. If the
model were the whole story, the skeptics would be right.

The grid is not for the model. It is for the arithmetic the design has to
survive on its way to the model, and that arithmetic is binary all the way
down. A binary computer can store a fraction exactly only when its
denominator is a power of two. Mathematicians call these the dyadic
rationals, and they are the machine's native numbers: floating point is
dyadic, fixed point is dyadic, and everything else is stored as an
approximation. Divide Virtua's 96-unit stem by its 1024-unit em and you get
0.09375, which a float holds verbatim. Divide the same stem by a 1000-unit
em and you get 0.096, which has no finite binary form; the machine keeps
0.0959999… and rounds at every step after. Knuth hit exactly this in TeX,
[watching 0.4 print back as 0.39999](https://research.swtch.com/fp-knuth),
and rebuilt the whole system on scaled points of 2⁻¹⁶ of a printer's point,
because dyadic fixed point was the only way to get the identical document
on every machine. The font stack made the same choice at every layer:
TrueType rasterizers compute in 26.6 fixed point, in sixty-fourths of a
pixel; variable fonts store every axis position in
[units of 2⁻¹⁴](https://learn.microsoft.com/en-us/typography/opentype/spec/otvarcommonformats);
and the OpenType specification itself
...
grid is the only design this stack writes down exactly rather than
approximates.
...
halving, doubling, scaling, and interpolating, and those are the four things
base two does perfectly.

Exactness compounds where the data factory runs. Interpolation is
`a + t · (b − a)`. When the masters sit on the 2-unit grid and the weights
are binary fractions, every intermediate value is dyadic, every machine
computes the identical result, and the only rounding in the whole pipeline
is the final snap back to the grid, the same snap everywhere. Run the same
factory on a decimal em at arbitrary weights and float error enters every
coordinate before the snap, differently on different hardware. One factory
emits deterministic, noise-free training data; the other emits data with a
faint hiss in it. There is even a right order to draw the samples: the
base-2 [van der Corput sequence](https://en.wikipedia.org/wiki/Van_der_Corput_sequence),
1/2, 1/4, 3/4, 1/8, 5/8, 3/8, 7/8, is dyadic at every step and evenly
spread at every prefix, so the oldest low-discrepancy sequence in numerical
analysis turns out to be a training schedule for fonts.

The raster world lines up too. Render a 1024-em glyph at 1024 pixels and
one font unit is one pixel; the dimension sheet at the top of this post is
drawn that way. The image models img2bez feeds on compress pictures through
an autoencoder that downsamples by eight, so their latent space is a
128 × 128 lattice in which each cell covers exactly an 8 × 8 block of font
units. The machine grid from the build log reappears, uninvited, inside the
diffusion model. Vector, raster, and latent are all counting in the same
base, and 2¹⁰ gives the design ten nested grids to count on, each level
containing the one below: the 64-grid of the metrics, the 8-grid of the
machine skeleton, the 2-grid of the human hand. The split of labor between
model and designer is a multiresolution decomposition, coarse structure and
fine detail, and coarse-to-fine is precisely the direction generative
models like to learn.
...
composed of discrete receptors, and visible light has a finite wavelength."
Then he states the design goal for automated letterform work, and it is
worth quoting at length because it is this project's build log written in
1986: "our goal should not be to make hand-tuning obsolete; it should
rather be to make hand-tuning tolerable. Let us try to create meta-designs
so that we would never want to change more than a few pixels per character,
say half a dozen, regardless of the resolution." A machine draft correct on
the coarse grid, a human budget of a half-dozen touches on the fine one:
that is the 8-grid and the 2-grid, except that in Virtua's pipeline every
one of those touches is also recorded, labeled, and learned from.
...
*Art of Computer Programming* calls balanced ternary "perhaps the prettiest
number system of all." But when he built typography systems that had to
produce the identical page on every machine, he did not build them on the
prettiest base. He built them on the base the machines speak.
But once the pipeline is computational end to end, from interpolation to
...
arithmetic. Section 09 admitted that some things are only sayable in form.
This one turns out to be sayable in arithmetic: on a binary machine, a
design drawn in powers of two is not encoded. It is simply written down.
