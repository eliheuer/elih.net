...
Virtua Grotesk is a typeface drawn on two nested grids. Structural
points go on the 8-unit grid. Optical corrections go on the 2-unit
grid. I call this a **self-labeling grid system**, and it makes the
typeface a machine-learning and data-engineering experiment as much
as a visual-design one. A small language model is now learning to
draw it. If you are reading this on [elih.net](/), you are reading
Virtua Grotesk right now.

Every measurement in the design defaults to a power-of-two based
grid system: the em
is 1024 = 2^10 units, chamfers are 16. The source files also hold
many values that are not powers-of-two but are short sums of
powers-of-two, like 64 + 32 = 96. This is by design. The
system prefers a power of two, never at the expense of the
neo-modernist letterform or its optical corrections. When a pure
power does not fit, the value is a short sum of powers: 64 is too
thin for a stem and 128 too heavy, so stems are 96 = 64 + 32. Where
96 is too light, 112 = 64 + 32 + 16. Sidebearings are 48 = 32 + 16.
Like the nested grid, the length of the sum is a free label: it
records how much optical correction a measurement needed. A Python
script in the font repo grades every measurement in both masters
against this system, and an AI agent fixes what the report flags in
a report, fix, render loop. The model will soon be on
Hugging Face as an open-weight release that anyone can use, run
locally (it's under 50 MB), and fine-tune. The font sources are
already on
...
powers-of-two down to the em itself: 1024 units, 2^10, rather than
...
and it is enforced, not aspirational: from the repo root, `make
...
points on the 8-grid with off-8 counted as the optical class,
advances on 8, and handle lengths and structural spans are scored as
short sums of powers-of-two. Point an
agent at that report and the agentic harness in the repo, and it can
be put to work reviewing and fixing whatever the grader flags.
...
set of recurring intervals instead of eighty-four bespoke ones,
...
stems, sidebearings, chamfers, and every value a tool fits or
generates. Optical corrections live on the 2-unit grid, and for now
only a person can judge them. Because everything structural is on 8,
a point on 2 but off 8 can only be a deliberate correction: the label
is in the coordinate itself, and nothing has to be annotated. In
practice the machine drafts on the 8-grid, a person corrects on the
2-grid, and the difference between the draft and the corrected
version is a labeled training set.
...
with no optical corrections, on the structure grid at every point.
...
of it needs the base to be two. But the model is only the middle of
the pipeline, and in the last section of this post I will argue that
base two is the best possible choice for a system like this.
...
verbs (`MOVE`, `LINE`, `CURVE`, `CLOSE`); conditioning tokens that name
the glyph and set its weight (the prompt, what to draw); and one token
for each legal position on the grid. (A fourth kind, the small
Bold-offset tokens of section 07, joins for two-master training.) Here
is the numeral **2** in the Regular weight, exactly as the codec in
the training repo emits it:
...
itself, stripped to the drawing, is the training data, about 90 tokens a glyph.
...
None of the machinery is new, and the lineage deserves naming.
...
For ML readers who want the card up front:
...
The protocol, for anyone who wants to hold it to a standard. The ten
...
filled glyphs. No FID, deliberately: a distribution metric over ten
glyphs would be noise dressed as rigor.
...
The training curves behind that table, parsed straight from the run
log so the figure cannot drift from the experiment: the pretrained arm
enters the finetune already below the control's best and stays below
it the whole way.
...
One more axis, because a reviewer will ask: how much hand-graded
data does the finetune actually need? Rerunning the same finetune
...
Grading more of the Bold master is the cheapest improvement on the
table.
...
The honest scoreboard, before anyone reads it for me: ten held-out
glyphs, one font family, and one training run per arm, so the error
bars above cover sampling variance, not retraining variance. A
five-fold split, every pair held out once, is queued for v0.9.
Section 08 reports what the same weights do off-family, including a
negative result. The
current ceiling is draft quality; the
division of labor is deliberate, the model proposing on the structure
grid and a person finishing on the correction grid. The deeper bet is
also still open. At this scale, on one machine, curated grid-native
data plus generic pretraining demonstrably beats a strong baseline,
and more graded data alone did not; whether that holds three orders of magnitude up, against
someone training a giant model on renders of every font in existence,
is precisely what the coming OFL-corpus scale-ups are for. And even in
that world the grid keeps its jobs: the encoding, the eval harness, and
...
could audit glyph by glyph. The grid-ifier is also the answer to the
fair charge that this system is boutique, one foundry's private
convention: any OFL family can be brought onto the grid in about a
minute, corrections and all.
...
data into the finetune, is queued for v0.9. Virtua-12M-0.8 is a
Virtua specialist, not a general boldening engine, and the table says
so plainly:
