...
<li><a href="#03-the-semantic-grid-hierarchy"><span class="n">03</span>The Semantic Grid Hierarchy</a></li>
...
The problem holding back machine learning for type design is data.
...
data engineers working with font sources. Today's font AI trains on
exactly this accidental data: scraped fonts, Google Fonts, and
proprietary datasets without strict engineering guidelines across
designs, heterogeneous in every dimension that matters.
...
collection assembled without it is many fonts in one place, not
training data capable of delivering useful models.

Modernism began as a reaction to a very similar situation. At the
1914 Werkbund congress in Cologne, Hermann Muthesius argued for
Typisierung, standard types for industrial production. Henry van de
Velde and the artists rejected it, and the fight nearly broke the
Werkbund. Jan Tschichold
wrote in 1928 that the use of ornament "comes from an attitude of
childish naivety." What these designers objected to was
not beauty but the designer's ego imposed on everyone else's visual
environment. Massimo Vignelli later called that visual pollution,
"degrading our environment just like all other types of pollution,"
and objected to work made "just to satisfy the ego of designers or
clients."

Josef Müller-Brockmann described his own method plainly: "In my
designs for posters, advertisements, brochures and exhibitions,
subjectivity is suppressed in favour of a geometric grid that
determines the arrangement of the type and images." Anonymous was his
word for the result, and he meant it as a compliment: "By anonymous I
mean neutral, rational typography which has nothing to do with banal
typography." Subjective interpretation, he held, falsifies the
message.

Tschichold changed his mind. The Nazis arrested him in Munich in
March 1933 and held him six weeks for "un-German" typography, and he
left for Basel that August. In 1946, answering Max Bill's charge that
he had betrayed modernism, he wrote that the New Typography's
"military will-to-order and its claim to sole power" belonged to the
same German inclination to the absolute that "unleashed Hitler's rule
and the Second World War." He spent his second career on classical
book typography: serif faces, centered titles, symmetrical pages, and
500 Penguin paperbacks. In 1967 he went back to the ornament line and
corrected it. "It is not so simple," he wrote. "The desire for
ornament is elemental and not childish-naive."

What he gave up was the dogma, not the engineering. Sabon, released
that same year, had to set identically on hand composition, Linotype
and Monotype at once. Linotype's duplexing forced roman, italic and
bold to share character widths. Monotype's matrix case rationed width
values, so letters had to come out a unit wider or narrower than he
wanted them. The brief also required the face to run 5 percent
narrower than Monotype Garamond.

That is the whole problem in one career. It is the problem I am
trying to solve with the models I am training. The grid was never the
enemy. Rigid rules were, because they could not hold the exceptions
the eye needs, and Tschichold could only resolve that by being a
master craftsman working inside a unit system. A model can learn the
system and the exceptions together, because the sources record both.
An old-style in the Plantin tradition is being drawn now with the
model trained on Virtua Grotesk. I think Tschichold would have
approved.

A century later the same argument is open again. Design now runs on
designer identity and on parasocial attachment to creators, more than
on engineered systems built for the people who have to live with the
result. AI slop is now flooding the visual environment, which makes
Vignelli's warning look prophetic.
...
drawing grid in their font editor and made it ten times coarser. FontLab
provides a 700-unit grid. Instead of using all 700 positions, they used
a 70-unit grid. Every node and Bézier handle had fewer legal positions.
The freedom was deliberate.
...
### 03. The Semantic Grid Hierarchy
...
1000. That is less exotic than it sounds. A
...
same logic down through every dimension. The full specification lives
in the repo as
...
that grades every glyph in both masters for compliance. Every point
must sit on the 2-grid. On-curve points should sit on the 8-grid, with
off-8 points reported by the QA tool. Handle lengths and structural spans
are scored as short sums of powers of two. When a pure power does not fit the
...
half of n, over 80 = 64 + 16, n plus a quarter of n. The report is
...
Large optical corrections sit on it as well. Only corrections finer
than 8 units drop to the 2-unit grid, and for now only a person can
judge those. A point on 2 but off 8 can only be a deliberate
...
The tiers are learnable. The evidence fits in three numbers.
...
Nobody labeled the tiers, and no auxiliary loss rewards them. The model
...
![The whole argument in one annotated specimen, drawn with designbot. Left: a zoomed crop of the word no sitting on the two-tier grid itself, with every point marked and the n's stem measured at 96 and the o's curve wall at 100. Right: 96 and 100 written in binary, followed by bars showing the share of points landing exactly on the 8-unit grid in held-out raw model output: chance 6 percent, Virtua-12M-v0.1 68 percent, human sources 85 percent.](./fig-semantic-grid.png)
...
![Two training-curve panels drawn with designbot from the v0.8 run log. Left: pretraining on 29,000 OFL pairs, validation loss on 290 held-out OFL pairs falling to a best of 1.53, faint train loss beneath. Right: the A/B on the same Virtua Grotesk validation set, the gray from-scratch control bottoming at 3.16 while the green fine-tune from pretrained weights starts lower and bottoms at 2.72, dots at every 500-step checkpoint and a ring on the shipped best checkpoint.](./fig-losscurve.png)
