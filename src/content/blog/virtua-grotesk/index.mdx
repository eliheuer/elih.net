...
When I shared this draft, the most common pushback was also the fairest
one: why powers of two? Any grid shrinks the coordinate vocabulary, so a
...
But the model is the middle of the pipeline, not the whole of it. The grid
...
For the machine-learning pipeline this post is about, the exactness pays
out four ways:
...
recorded human optical correction. The lattice is the labeling function
for the judgment dataset in the build log; the annotation pass costs
nothing because the grid is the annotator.
...
this post is drawn that way. The image models img2bez feeds on compress
...
block of font units. The machine grid from the build log reappears,
uninvited, inside the diffusion model. And 2¹⁰ gives the design ten nested
grids, each level containing the one below, the 64-grid of the metrics,
the 8-grid of the machine skeleton, the 2-grid of the human hand, where a
decimal em offers three. Coarse-to-fine is the direction generative models
like to learn, and binary builds the finest ladder.
...
and it is worth quoting at length because it is this project's build log
written in 1986: "our goal should not be to make hand-tuning obsolete; it
...
opaque. Aesthetics is the whole game. If it were not, system fonts exist,
and there would be better things to do with a life. The powers of two
