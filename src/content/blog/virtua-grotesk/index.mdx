...
1. **Normalize exactly.** Dividing a coordinate by the em must give a
   ...
   height, x-height) must land on whole coordinates.
...
3. **Compute the same everywhere.** The same font at the same weight
   must produce identical coordinates on every machine.

One comes from the labeling scheme:

4. **Read the label from the coordinate.** A point's grid level must be
   computable from the number alone, which forces the grids into a
   chain, each nested strictly inside the next, and favors a deep chain
   over a shallow one.
...
font across implementations." Exact values meet the constraint for
free: where nothing rounds, nothing can round differently. Approximate
values meet it only by decree, one prescribed rounding at a time.
...
One more alignment is particular to this pipeline rather than imposed
by the formats. At an arbitrary text size no em lines up with the
pixels; a rasterizer scales every outline into
[1/64-pixel fixed point](https://freetype.org/freetype2/docs/glyphs/glyphs-6.html)
and renders 13-pixel type the same way whatever the em. But at the
sizes this pipeline chooses, the grids coincide. Render a 1024-em glyph
at 1024 pixels and one font unit is one pixel (the specimen atop this
post is drawn that way). The image models feeding img2bez downsample by
eight, so the 8-unit structure grid reappears as their 128 × 128 latent
lattice. The design grid, the pixel grid, and the model's grid are one
lattice.
