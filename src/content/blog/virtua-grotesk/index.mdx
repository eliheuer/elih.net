...
failed." The model failure is a data problem, upstream of any model. The
bad vectorization I fixed and wrote about in
[another blog post about my img2bez tool](/blog/img2bez), which places
points on a glyph's structure, not just its silhouette, and puts outlines
straight onto the 2-unit grid. It runs throughout the pipeline and harness
that Virtua Grotesk uses.
...
must be drawn again heavier, same structure. Given the Regular, the model
draws the Bold. The goal is a workflow where a designer draws the core
control glyphs by hand (H, O, n, o, e, a, R) and the model bootstraps the
rest (the remaining glyphs and the second master), with the designer
correcting on the 2-unit grid.

Font tools already interpolate, exactly and for free, when both masters
exist. The model is for the case they can't touch: the second master
doesn't exist yet. The classical fallbacks, the offset-curve filter
(thickens mechanically, clogs the joins) and extrapolation or RMX-style
scaling, approximate a new weight from masters you already have; the
model learns it from a corpus instead. Every result below is scored on
this missing-master case, Bolds the model never saw.
...
Virtua-12M-v0.1 beats the baseline, and a controlled comparison shows
why. Trained only on the graded corpus, it tied the baseline. Pretrained
on 29,000 Google Fonts pairs and then fine-tuned on the same corpus, it
beats the baseline on all ten held-out glyphs, every metric. Cleaning the
data kept the tie; other designers' letterforms broke it.

Ten glyphs were set aside before training; their Bolds were shown to
neither run. Each number is the mean ± spread of five samples, in font
units. MAE is average coordinate error; Chamfer scores the shape along the
outline rather than point placement; IoU is shared ink. (FID needs far
more than ten samples, so it's omitted.)
...
How much graded data does the fine-tune need? Cutting the corpus down:
twelve pairs score 28.0 MAE, twenty-five 25.5, fifty 24.0, every seed
beating the baseline's 31.3. The curve isn't saturated, and even twelve
pairs on pretrained weights beat fifty from scratch. Grading more of the
Bold master is the cheapest next win.

The review sheet shows six held-out glyphs: human Regular in, model
Bold out, human Bold beneath for reference.

![Held-out glyph boldening test for Virtua-12M-v0.1. For K, E, M, n, b, and c, the red top row shows the manually drawn Regular input, the orange middle row shows the model's Bold output, and the green bottom row shows the manually drawn Bold reference.](./fig-model-review.png)

One transfer up close, the held-out `n`: the hand-drawn Regular, and
beside it the Bold the model drew from it.

![The lowercase n twice on its real 8-unit design grid, drawn with designbot in the Runebender point language. The red hand-drawn Regular sits on the left. An arrow points to the green Bold drawn by Virtua-12M-v0.1 on the right.](./fig-model-bolden-n.png)

This is a small test (ten glyphs, one font, one run per condition), and
the numbers will change as the work grows. The output is a draft; a person
still corrects it on the 2-unit grid. Whether the approach holds at much
larger scale is what the next runs are for. Section 08 reports what the
same model does on fonts it wasn't tuned for, including a result that
didn't work.
...
(Section 02), so the direction now is to retire the borrowed fonts
...
current workaround fakes with thousands of glyphs and rules what a hand
does in one motion. A generative font draws each glyph in context. The
