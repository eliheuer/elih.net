...
Virtua Grotesk is a typeface designed to be training data. Its
drawing grid comes in nested tiers that label every point as it is
placed: structure on the coarse tier, the eye's optical corrections
on the fine one. I call this a **self-labeling grid system**, and it
makes the typeface a machine-learning and data-engineering experiment
as much as a visual-design one. A small neural network is now
learning to draw it. If you are reading this on [elih.net](/),
Virtua Grotesk is the typeface you are reading right now.

Everything is built on powers-of-two, and every dimension reaches
for an elegant binary number: the structure grid is 8 units, the
correction grid 2, the em 1024 = 2^10, stems 96, chamfers 16,
sidebearings 48 (32 + 16), and so on. The model will soon be on
...
The four annotated specimens below are measured directly from the
sources, every dimension line anchored to real outline coordinates,
so unlike a table they cannot drift out of date. They are drawn in
the Runebender point language: circles are smooth on-curve points,
squares are corners, small circles are the bézier handles, and the
color of every point is its tier. Green points sit on the 8-unit
structure grid; red points sit off 8 on the 2-unit correction grid:
the optical corrections. Purple numbers are handle lengths from the
favored set. First the whole Latin system, then the lowercase up
close, then the two masters side by side, and the same system
carrying Arabic:
...
The division of labor between the tiers is the whole system.
Structure belongs on 8: stems, sidebearings, chamfers, and every
offset a tool fits or generates. Dropping to 2 is an optical
correction, the small nudge the eye asks for and, for now, only a
person can judge. The grid is a starting point, not a cage, and the
labeling costs nothing: because everything structural lives on 8, any
point on 2 but off 8 is a deliberate correction, with nothing to
annotate. That is all the self- in self-labeling means. The machine
proposes structure on 8, the human teaches optics on 2, and the
difference between a draft and its corrected version is a clean
...
This is the **self-labeling grid**, and it, not any single number, is
the core idea of this post. Which tier a value sits on is its meaning. The n's stem is 96: structure, so it lives on the 8-unit
...
are just the base in which this ladder of tiers runs deepest, and,
separately, the numbers I find beautiful.

The tiers are also learnable, and that is measurable. The model's
vocabulary enforces only the bottom tier: every coordinate token is a
...
every point. A fair objection: a next-token model that reproduces the
statistics of its training data is doing exactly what it should,
nothing more. Agreed, and that is the point. The tiers are learnable
as plain statistics, with no labels and no auxiliary loss, and not only
as frequencies: where the human sources step off the structure grid,
the model tends to step off at the same points.
...
matter of taste. Section 09 makes the case that the rest of the pipeline
disagrees.
