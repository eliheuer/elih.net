...
The model also runs inside
[Runebender](https://runebender.org), the font editor. Below, I
...
green-graded glyphs already in the sources as reference. Real time,
37 seconds:
...
compete with it. The model is for the case interpolation cannot
touch: the second master does not exist yet. Every result below is
scored on that case, Bolds the model never saw. The baseline is the
classical answer to the same missing-master problem: shift every
point by the average Bold offset of the pairs you do have. The
offset-curve filter, the other tool that can draw a Bold from one
master, is queued as the next baseline.
...
long-range problem and becomes local prediction. At generation time
the Regular coordinates are *forced* from the input and the model
fills only the Bold slots, so the output cannot structurally diverge
from its Regular. The encoding guarantees master compatibility.
...
v0.8), is the first to beat that baseline, and the experiment that
produced it was a clean A/B. Arm one took the
...
the same architecture on 29,000 glyph pairs traced from OFL fonts in
the Google Fonts collection, then finetuned on the same graded
corpus: it beats the baseline on all ten
...
data into the finetune, is queued for v0.9. Virtua-12M-v0.1 is a Virtua specialist, not a general boldening
engine:
...
4. Boldening collapses to choosing among familiar offsets.
