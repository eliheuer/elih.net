...
Boldening is the task with immediate production value. Every glyph
drawn once must be drawn again heavier, with the same structure.
Given the Regular, the model draws the Bold.
...
yet. Every result below is scored on that case, Bolds the model never
saw. The baseline: shift every point by the average Bold offset of
the pairs you do have. The offset-curve filter, the other tool that
can draw a Bold from one master, is queued as the next baseline.
...
master never had a real `a`; the slot held an unboldened copy of the
