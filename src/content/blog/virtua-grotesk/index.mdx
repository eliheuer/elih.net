...
master? No. Interpolation is halving, and a power-of-two grid is
closed under halving: the midpoint of two grid points is on the grid,
or one level finer down the same ladder, exactly. A 1000-unit em is
not closed: halving a 10-unit interval gives 5, then 2.5, then 1.25,
values off any clean level that must be rounded back, a small error in
every sample. This is why a font built to be training data belongs on
1024, not 1000: one em makes noise-free data, the other cannot.
Section 09 proves the general case.
