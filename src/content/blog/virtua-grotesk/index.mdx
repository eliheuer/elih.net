...
A digital font is not a set of pictures, it is a small program. Massimo
Vignelli called design a language and meant it as an analogy. In type the
analogy is literal: a glyph is a sentence in a formal language, built from
points and curves the way a sentence is built from words.

And the source is code you can read. A glyph is stored as text, a short list of
instructions: start here, curve to there, close the path. Around the outlines
sit layers that are unambiguously programs, a rule language that substitutes and
repositions glyphs in context, an interpolation system that computes the
in-between weights on demand. The whole thing is compiled from plain-text
sources by a build tool, kept in git, and executed by the shaper and rasterizer
every time a letter appears on screen. Program is not a metaphor, it is the file
format.

This is not a new idea, and its clearest early form is a book. In 1964 Karl
...
but to draw them regularly enough that a program can learn them.
...
fonts, or Google Fonts, heterogeneous in every dimension that matters. Virtua
Grotesk is designed the other way, on purpose.
...
grid their font editor provided and made it ten times coarser. FontLab's
standard cap height is 700 units, and instead of drawing on that 700-grid
they allowed themselves a 70-grid. In a region that would normally offer 121
positions to place a point, an 11-by-11 square, the coarser grid left four.
Fewer legal positions for every node and every bézier handle. Deliberately
less freedom.
...
Replica has ancestors of its own. Wim Crouwel's New Alphabet (1967) built
every letter on a strict grid of horizontals and verticals, grid-as-constraint
decades before the bézier. What Replica added, and what matters here, is that
the constraint lives in the vector outline itself. That is the line between
this and pixel type: Emigre's Lo-Res or the screen-era faces of Matthew Carter
are grid-bound too, but there the grid is the pixel, a raster cell, not a
coordinate lattice the curves are drawn on. A coarse grid governing where the
nodes and handles of a bézier may land is rarer, and it is the part Virtua
inherits.
