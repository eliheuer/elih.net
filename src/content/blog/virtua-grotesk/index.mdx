...
directly after its Regular partner, encoded as a small offset (the
Bold stem is the Regular's +96). At generation time the Regular
coordinates are *forced* from the input and the model fills only the
offset slots, so the output cannot structurally diverge from its
Regular.
