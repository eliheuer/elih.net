...
grid system. The source outlines also hold
many values that are not powers-of-two but are short sums of
powers-of-two, like 64 + 32 = 96. The system prefers a power of
two, but never at the expense of the aesthetics of neo-modernist
letterforms or optical corrections. When a pure
power-of-two does not fit, the value is a short sum of them: if 64 is too
light and 128 too heavy, 96 (64 + 32) might be used. Where
96 is too light, 112 (64 + 32 + 16) might work better, and so on.
Like the nested grid, the number of powers-of-two needed to reach a
value is a free label and a visual design constraint: 128 needs one,
96 needs two, 112 needs three.
A Python script grades every measurement against this system, and an AI agent harness system
fixes what the report flags in a report, fix, render loop.

The model will soon be on Hugging Face as an open-weight model that anyone can use, run
locally, and fine-tune. The font sources and harness system are
already on
...
held-out glyphs whose Bolds it never saw, Virtua-12M-0.8 lands on it
...
A transformer works from a fixed *vocabulary*: the set of tokens it
can read and write. Here that set holds three kinds of token: the
four drawing verbs (`MOVE`, `LINE`, `CURVE`, `CLOSE`); conditioning
tokens that name the glyph and set its weight; and one token for each
legal position on the grid. A fourth kind, the Bold-offset tokens,
joins in section 07. Here is the numeral **2** in the Regular weight,
exactly as the codec emits it:

```
BOS N_two W400 ADV 592
MOVE 48 0
LINE 528 0
LINE 544 16
LINE 544 72
...
the glyph (the digit two), `W400` sets
the weight (Regular), and `ADV 592` is its advance width,
...
stem is 96 every time, never 95 or 97, so structural tokens recur,
and the optical corrections sit off the 8-grid, so their tokens stay
rare. To the model, a correction is a rare, high-information token.
...
homogeneous, then ask how small the model can get. [Simon
...
| vocabulary | 1,784 tokens: drawing commands, glyph names, 2-grid coordinates, Bold deltas |
| pretraining | 29,000 Regular-Bold glyph pairs traced from OFL fonts |
...
Every row comes from the run log and config checked in with the run.
...
commands one token at a time, and a single training stage runs in a
45-minute box on an M4 Pro or gaming GPU.
...
An obvious objection first: font tools already interpolate. When both
masters exist, interpolation is exact and free, and no model should
compete with it. The model is for the case interpolation cannot touch:
the second master does not exist yet. Every result below is scored on
exactly that case, Bolds the model never saw, and the baseline it must
beat is a strong classical answer to the same missing-master problem:
shift every point by the average Bold offset learned from the pairs you
do have. Beating it means beating a serious non-learning answer to
the same problem; the classical offset-curve filter, the other tool
that can draw a Bold from a single master, is queued as the next
baseline.
...
v0.7 recipe and retrained it on the freshly graded corpus (50 approved
Bold pairs, up from 39): it tied the baseline again. Arm two pretrained
the same architecture on 29,000 glyph pairs traced from OFL fonts, then
finetuned on the same graded corpus: it beats the baseline on all ten
held-out glyphs, on every metric's seed-averaged score. Data cleaning alone kept the tie;
other designers' letterforms broke it, which is section 08's data plan
reporting its first result.

The protocol. The ten
held-out pairs (six carried over from v0.7, plus two capitals and two
lowercase drawn at random) were frozen before training, and their
Bolds never trained in either arm. Each model is sampled at
temperature 0.3 with five random seeds; the table reports mean ±
standard deviation across seeds, and the baseline is deterministic.
Alongside the recipe's raw MAE (mean absolute error per coordinate,
font units), two metrics the vector-graphics literature will
recognize: symmetric Chamfer distance on densely sampled outlines,
because control-point error rewards matching the construction while
Chamfer measures the drawn shape, and intersection-over-union of the
filled glyphs. No FID: over ten glyphs a distribution metric is noise.
...
| + OFL pretrain (Virtua-12M-0.8) | **24.0 ± 1.6** | **22.4 ± 1.3** | **0.745 ± 0.016** |
...
How much hand-graded data does the finetune need? Rerunning the same finetune
from the same pretrained weights with the graded corpus cut down:
twelve pairs score 28.0 ± 1.9 MAE, twenty-five score 25.5 ± 0.9, and
the full fifty score 24.0 ± 1.6, every seed of every run better than
the baseline's 31.3. The curve is monotone in all three metrics and
not yet saturated, so graded pairs are still buying accuracy, and
even twelve of them on pretrained weights beat fifty from scratch.
Grading more of the Bold master is the cheapest next improvement.

The review sheet shows six of those held-out glyphs and
...
design has ever had. It is not finished, but it is a draft where there was nothing: v0.7's attempt
...
retraining variance. A five-fold split, every pair held out once, is
queued for v0.9. Section 08 reports what the same weights do
off-family, including a negative result. The current ceiling is draft
quality, and the division of labor is deliberate: the model proposes
on the structure grid, a person finishes on the correction grid. The
deeper bet is still open. At this scale, curated grid-native data
plus generic pretraining beats a strong baseline, and more graded
data alone did not. Whether that holds three orders of magnitude up,
against a giant model trained on renders of every font in existence,
is what the OFL-corpus scale-ups are for. Even in that world the grid
keeps its jobs: the encoding, the eval harness, and the labeled
corrections survive any size of model.
...
at OFL fonts, this is the long-term data plan: a growing corpus of
grid-native families, every one OFL, with a provenance manifest I can
publish. No scraping, no gray areas: training data you could audit
...
boldens glyphs it has never seen. The honest scope: the held-out unit
...
data into the finetune, is queued for v0.9. Virtua-12M-0.8 is a Virtua specialist, not a general boldening
engine:
...
powers-of-two? So I added this section, which tries to prove, not just
defend, that this is the right grid system for machine-learning data
engineering in type design. Any grid shrinks the coordinate vocabulary, so a
...
The engineers arrive at the same arithmetic by their own road. Karpathy
sorts model dimensions into nice numbers, 768 and 1024, and ugly ones
like 50,257; tuning [nanoGPT](https://github.com/karpathy/nanoGPT), he
found a 25 percent speedup just by padding that ugly vocabulary up to
50,304, the nearest multiple of 64, and signed off,
...
**Close under the operations.** This is the keystone. Ask which
numbers you can reach from
...
and its grid level is simply the
...
the reason protractors read to 360, buy exact thirds at the price of a
hierarchy that is a lattice instead of a chain, and a denominator
binary arithmetic cannot hold; the thirds they sell are the divisions
the eye corrects anyway. The modernists' own candidate fails harder:
Le Corbusier scaled the Modulor to the golden section and the human
body, and φ is irrational, so nothing normalizes exactly, ever.
Corbusier matched his grid to the measure of man. This grid matches
...
pixels and one font unit is one pixel; the annotated specimen at the top of
...
rules type design anyway: aesthetics. Typographic modernism was never
especially concerned with powers-of-two, though it is not innocent of
them: A-series paper halves at every fold. Had the computer arrived a
little earlier, the Swiss might have fallen for base two the way I
have; their creed was truth to materials, and the true material of the
computer is the bit. The same taste is at work all the
