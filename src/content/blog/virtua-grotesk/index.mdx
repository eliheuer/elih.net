...
system**. The drawing grid comes in 8-unit and 2-unit nested tiers.
The tier a point lands on records what kind of decision it was:
structure on the 8-unit grid, optical correction on the 2-unit grid.
Everything is ruled in powers-of-two, and every dimension reaches for
an elegant binary number: 1024 = 2^10 units per em, stems of 96,
chamfers of 16, sidebearings of 48 (32 + 16), and so on. The base is part of the
labeling too: in binary, a value's tier can be read straight off its
low digits. If you are reading this on [elih.net](/), Virtua Grotesk
is the typeface you are reading right now.

It is also a machine-learning experiment. The same grid that gives the
font its aesthetic makes its source files unusually good training
data, and a small neural network is now learning to draw it. The model
will soon be on Hugging Face as an open-weight release that anyone can
use and fine-tune on their own fonts. The font sources are on
[GitHub](https://github.com/eliheuer/virtua-grotesk), free under the
SIL Open Font License (OFL) v1.1. This post is about the typeface, the
neural network, and why they are the same idea.
...
glyphs, one font family, a single training seed, no error bars. These
numbers will wobble, and the current ceiling is draft quality; the
