...
`n`, interpolated from Regular to Bold at 1/4, 1/2, and 3/4. Every
intermediate is a genuine grid-native glyph: the stem steps 96, 120,
144, 168, 192, and every point lands on a grid intersection.

![Five n outlines drawn with designbot, the n interpolated from Regular to Bold at 1/4, 1/2, 3/4, each on its own 8-unit grid with on-curve and off-curve points marked. Points are colored by grid level, green on the 8-unit structure grid and red off 8 on the 2-unit correction grid. The left stem is dimensioned under each weight: 96, 120, 144, 168, 192, every value on the grid, and each intermediate n is a clean, grid-native glyph.](./fig-interp-outlines.png)
