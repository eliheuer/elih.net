...
The encoding makes the task local. The codec interleaves the two
masters point by point, so each Bold value is predicted as a small
offset from the Regular value directly before it (the Bold stem is
the Regular's +96). The Regular coordinates are *forced* from the
input, so the output cannot structurally diverge from its Regular.
