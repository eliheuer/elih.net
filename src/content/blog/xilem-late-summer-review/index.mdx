...
## Why it still does not look the same
...
**A missing primitive can cost a feature, not just lines.** The GPUI
build's floating metrics panel has editable fields: you set the
sidebearings and the width in the panel, over the drawing. Mine cannot,
and the reason is a chain. The panel floats, so in view code it wants a
...
input, and there is no view for a layer, so the only way back to an
editable panel is to hand-write text editing inside my widget. One
missing overlay primitive turned an ordinary feature into a project.
...
## Reading this against the project's own review
...
the sidebar with folding groups, the inspector, glyph renaming and
metrics editing from the overview, tabs with independent sessions,
